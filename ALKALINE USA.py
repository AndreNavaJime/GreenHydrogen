...
# ========== NEW: ELECTRICITY PRICE SCENARIOS (per Supplement baseline & bracketing) ==========
ELECTRICITY_PRICE_SCENARIOS = {
    'conservative': {
        'decline_pct': 0.10,      # 10% decline 2022→2060 (slow decarbonization)
        'description': 'Slow grid decarbonization'
    },
    'moderate': {
        'decline_pct': 0.30,      # 30% decline 2022→2060 (paper baseline)
        'description': 'Baseline electricity price decline (Table S3 default)'
    },
    'aggressive': {
        'decline_pct': 0.50,      # 50% decline 2022→2060 (rapid VRE + learning)
        'description': 'Aggressive grid decarbonization + VRE cost learning'
    }
}

# USER CONTROL: Set which price scenario to run (run the script 3x to generate all)
...
print("\n" + "="*70)
print("ALKALINE (AWE) TECHNO-ECONOMIC ASSESSMENT — ACTIVE SCENARIO")
print("="*70)
...
# Example (Oklahoma) to show 2022→2060 price path
try:
...
# NEW: scenario-tagged filenames
outfile_main   = os.path.join(save_dir, f"USAALKALINE_price_{price_scenario}.csv")
overlay_file   = os.path.join(save_dir, f"USAALKALINE_price_{price_scenario}_OVERLAYS.csv")
flips_file     = os.path.join(save_dir, f"USAALKALINE_price_{price_scenario}_RANK_FLIPS.csv")
