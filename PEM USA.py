# =============================================================================
#  PEM USA.py – Spyder-ready with decade-step projections
#  Adds:
...
#   (6) Rank-flip & overlay exports also scenario-tagged
...
# ---------- 0.1 Global TEA toggles ------------------------------------------#
PLANT_LIFE_YEARS        = 20
DISCOUNT_RATE           = 0.08
OPEX_RATIO              = 0.05     # per supplement
OPEX_FACTOR             = 1.0
WITH_REPLACEMENT        = False    # keep False for PEM unless you want to test it
STACK_LIFE_HOURS_PEM    = 0        # unused unless WITH_REPLACEMENT=True
REPLACEMENT_COST_FRACT  = 0.0

# Delivery/Storage overlays
SCENARIOS = {
...
# Overlay sanity check for a typical distance (e.g., 10 km)
_test_d = 10.0
_print = lambda name, p: print(f"{name}: {_test_d*p['per_km'] + p['storage']:.5f} $/kg")
print("Overlay sanity @ 10 km:")
for k, v in SCENARIOS.items():
    _print(k, v)


# ========== NEW: ELECTRICITY PRICE SCENARIOS (baseline + brackets) ==========
...
# Resource index → CF in [0.6, 0.9] via proxy
...
# ========== NEW: CF DIAGNOSTIC (Eq. 7 Validation) ==========
print("\n" + "="*70)
...
# ---------- 5. PEM trajectories (Table S3/S4) --------------------------------
...
# ========== NEW: PRINT ACTIVE SCENARIO HEADER ==========
print("\n" + "="*70)
print("PEM TECHNO-ECONOMIC ASSESSMENT — ACTIVE SCENARIO")
print("="*70)
...
for _, r in df.iterrows():
    ...
    for y in years:
        ...
        # ========== NEW: scenario-based electricity price path ==========
...
# Scenario-tagged filenames
outfile_main = os.path.join(save_dir, f"USAPEM_price_{price_scenario}.csv")
overlay_file = os.path.join(save_dir, f"USAPEM_price_{price_scenario}_OVERLAYS.csv")
flips_file   = os.path.join(save_dir, f"USAPEM_price_{price_scenario}_RANK_FLIPS.csv")
