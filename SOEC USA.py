...
def read_flexible(path, ncols, cols):
    ...
    if df.shape[1] == 1:
        t = df.iloc[:,0].str.split(",", expand=True)
        if t.shape[1] < ncols:
            t = df.iloc[:,0].str.split(r"\s+", expand=True)
        df = t
    df = df.iloc[:,:ncols]
    df.columns = cols
    return df
...
water = clean_state(
    pd.read_csv("USWATER.csv").rename(
...
# ---------- 2.1 CO₂ intensity -----------------------------------------------#
co2["CO2_avg_18_23"] = co2[co2cols].loc[:, [c for c in co2cols if 2018<=int(c)<=2023]].mean(axis=1)
co2["CO2_intensity_norm"] = co2["CO2_avg_18_23"] / co2["CO2_avg_18_23"].max()
co2 = co2[["State","CO2_intensity_norm"]]

# ---------- 2.2 Merge -------------------------------------------------------#
frames = [(ghi,"GHI"),(wind,"WIND"),(land,"LAND"),(infra,"INFRA"),
...
# ---------- 3.1 Transport & Storage Cost -----------------------------------#
df["Transport_km"] = 200  # Estimation
...
for _,r in df.iterrows():
    ...
    for y in years:
        ...
        kgH2 = cf * 8760 / eff
        lcoh = cap*crf/kgH2 + cap*opex_ratio*(cap/cap22)/kgH2 + p_e*eff + wc + cs + tc + sc
        rows.append([s,y,round(cf,3),round(cap,2),round(eff,2),
