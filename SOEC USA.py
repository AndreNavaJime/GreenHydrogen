# =============================================================================
#  SOEC USA.py – Decade-step LCOH model for SOEC (2022, 2030, 2040, 2050, 2060)
#  • Table S3 values (discount=0.08, life=20, OPEX=0.05)
#  • Robust CO₂ loader + guarded merge (no silent zeros)
#  • Plant-gate + Low/Med/High overlays (equal to baseline by default)
...
save_dir = r"C:\Users\Navar\OneDrive\Documentos\USA"     # <- unified save folder
os.makedirs(save_dir, exist_ok=True)
os.chdir(data_dir)

# ---------- 0.1 TEA toggles (neutral by default) ----------------------------#
PLANT_LIFE_YEARS        = 20
DISCOUNT_RATE           = 0.08
OPEX_RATIO              = 0.05
OPEX_FACTOR             = 1.0
WITH_REPLACEMENT        = False      # keep False unless modeling replacements
STACK_LIFE_HOURS_SOEC   = 0          # set (>0) if you enable replacements
REPLACEMENT_COST_FRACT  = 0.0

# Delivery/Storage overlays – equal to your original baseline by default
SCENARIOS = {
...
def load_co2_robust(path_csv, tech_factor):
    ...
    # identify year columns (2018–2023), salvage messy headers if needed
    ...
    co2["co2_surcharge_$kg"]  = co2["CO2_intensity_norm"] * float(tech_factor)  # SOEC factor = 0.05
...
# Water
_ = pd.read_csv("USWATER.csv", header=None)  # preview if needed
...
# ---------- 3.2 CO₂ load (SOEC factor = 0.05) + guarded merge ---------------#
...
# Keep only 48 contiguous states
contig_48 = {
...
# Transport distance for overlays
df["Transport_km"] = df["distance_to_grid_km"].fillna(200)

# ---------- 5) SOEC trajectories (Table S3) ---------------------------------#
cap22, cap26, cap31 = 2500, 500, 200   # $/kW
sec22, sec26, sec31 = 38, 36, 35       # kWh/kg (electrical)
sec_floor = 34
...
if flip_rows:
    ...
    flips_file = os.path.join(save_dir, "USASOEC_DELIVERY_RANK_FLIPS.csv")
    flips.to_csv(flips_file, index=False)
    print(f"📝 Delivery/storage ranking flips saved → {flips_file}")

overlay_file = os.path.join(save_dir, "USASOEC_OVERLAYS.csv")
