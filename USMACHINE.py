...
with tabs[3]:
    ...
    # Use consistent scenario inputs for all states
    if calculation_mode == "ML Model":
        # Capture fixed scenario values once
        cap_map = cap_dynamic
        eff_map = eff_dynamic
        elec_map = elec
        ...
        for st_name in sorted(df.State.unique()):
            ...
            row = {
                "CF": cf_val,
                "CAPEX_$/kW": cap_map,
                "Efficiency_kWh_kg": eff_map,
                "Electricity_$/kWh": elec_map,
            ...
            # Encode the year for ML model input
        ...
        # Align with model input format
        df_features = pd.get_dummies(df_states.drop(columns=["State"]))
        df_features = df_features.reindex(columns=results[model_name]["features"], fill_value=0)

        df_states["LCOH ($/kg)"] = best.predict(df_features).round(2)
