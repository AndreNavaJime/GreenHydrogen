...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        ...
        # Store State separately to reattach after prediction
        df_states = df_inputs[["State"]].copy()

        # Align to model features (drops State)
        df_features = df_inputs.drop(columns=["State"])
        df_features = df_features.reindex(columns=results[model_name]["features"], fill_value=0)

        # Predict and rebuild DataFrame
        df_states["LCOH ($/kg)"] = best.predict(df_features).round(2)
        merged = gdf.merge(df_states, on="State", how="left")
