...
# ─────────────────────────────────────────────────────────────────────────
# TAB 2  –  Feature Importance, SHAP & Trends (now with calibration diagnostics)
# ─────────────────────────────────────────────────────────────────────────
with tabs[2]:
    ...
    def _white_canvas(fig):
        ...
        if hasattr(fig, "update_coloraxes"):
            fig.update_coloraxes(colorbar_tickcolor="black",
    ...
    if ml_mode and results and model_name in results:
        ...
        if shap_vals is not None and explainer is not None and X_train is not None and len(X_train) > 0:
            ...
            plt.figure()
            _shap.summary_plot(shap_vals_plot, X_train_plot,
        ...
        st.info("Turn on **ML Model** mode to view SHAP.")

    # ───────────────────────────── Calibration diagnostics ───────────────
    st.markdown("### Model Calibration & Error Diagnostics")
    if ml_mode and results and model_name in results:
        src = st.radio("Prediction source", ["Hold-out (fast)", "5-fold OOF (no leakage, slower)"],
                       horizontal=True, key="diag_src")

        # Default: use stored hold-out predictions
        y_true = np.array(results[model_name]["y_true"], dtype=float)
        y_pred = np.array(results[model_name]["preds"], dtype=float)
        note = "hold-out"

        # Optional: compute out-of-fold predictions using current estimator's params
        if src.startswith("5-fold"):
            from sklearn.model_selection import KFold, cross_val_predict
            # Rebuild full X with the SAME schema as training
            df_full = df.copy()
            ...
            # Align to feature order used by the selected model
            feats = results[model_name]["features"]
            for col in feats:
        ...
        # Parity plot with equality + linear fit
        ...
        fig_par = px.scatter(df_par, x="Measured", y="Predicted", opacity=0.7)
        ...
        fig_par.update_layout(
            title=f"Parity plot ({model_name}, {note}) — fit: ŷ = {intercept:.3f} + {slope:.3f}·y,  R²={r2_parity:.3f}",
            xaxis_title="Measured LCOH ($/kg)", yaxis_title="Predicted LCOH ($/kg)",
        )
        _white_canvas(fig_par)
        ...
        with st.expander("Residual QQ plot (optional)"):
            try:
                ...
                st.info(f"QQ plot unavailable: {e}")
    else:
        st.info("Turn on **ML Model** mode to view calibration diagnostics.")
