...
if calculation_mode == "ML Model":
    ...
    auto_mode = st.sidebar.checkbox("Use DOE projections?", value=True)
...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        ...
        # Load True Equation CSV summary
        true_eq_summary = pd.read_csv("2025-07-12T03-22_export.csv")  # Make sure path is correct
        merged = gdf.merge(true_eq_summary, on="State", how="left")

        fig_map = px.choropleth(
            merged, geojson=merged.geometry,
            locations=merged.index, color="mean",
            hover_name="State", color_continuous_scale="Viridis",
            template="plotly_dark",
            title="Predicted LCOH by State (True Equation, Year 2022)"
        )
        fig_map.update_geos(scope="usa")
        fig_map.update_coloraxes(colorbar_tickformat=".2f")
        st.plotly_chart(fig_map, use_container_width=True)
        ...
        st.warning("🌐 This map is only available in **ML Model** or **True Equation** mode.")
