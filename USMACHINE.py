...
def train_models(
    df: pd.DataFrame,
    *,                     # force keyword-only
    learners=("LightGBM", "GBRegressor", "RandomForest"),
    run_shap: bool = False,
    run_cv: bool = False,
    verbose_sidebar: bool = True,
):
    """
    Fit selected regressors and (optionally) compute SHAP and 5-fold CV.
    ...
    # Optional imports (only used if requested)
    try:
        from xgboost import XGBRegressor
        _HAS_XGB = True
    except Exception:
    ...
    # ---------- 3) Keep rows with finite y; align groups ----------
    mask = np.isfinite(y)
    ...
    if groups is not None:
        ...
        if overlap and verbose_sidebar:
    ...
    if _HAS_CAT:
        all_defs["CatBoost"] = CatBoostRegressor(
            iterations=500, depth=8, learning_rate=0.05, l2_leaf_reg=3.0,
            subsample=0.8, rsm=0.8, verbose=0, random_state=42
        )

    # filter to requested learners that we actually have
    ...
    for name, model in models.items():
        ...
        # --- CV (optional; group-aware when possible) ---
        try:
            if run_cv:
