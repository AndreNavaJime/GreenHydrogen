...
def train_models(df: pd.DataFrame):
    ...
    # drop target & non-features; one-hot Tech
    X = pd.get_dummies(
    ...
    # ---------- 3) Keep only rows with finite y (and align groups if present) ----------
    mask = np.isfinite(y)
    ...
    # ---------- 4) Train/test split (group-aware by State to prevent leakage) ----------
    ...
    for name, model in models.items():
        ...
        # --- SHAP (clean background & prediction slices) -------------------
        ...
        try:
            if groups is not None:
                # use training portion only, grouped by state
