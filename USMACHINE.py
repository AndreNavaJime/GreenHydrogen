...
def train_models(df: pd.DataFrame):
    ...
    # 2) Force numeric (fix strings like "[4.144E0]")
    X = X.replace(r'^\s*\[(.+)\]\s*$', r'\1', regex=True)
    X = X.apply(pd.to_numeric, errors="coerce")
    X = X.replace([np.inf, -np.inf], np.nan)
    all_null_cols = [c for c in X.columns if X[c].isna().all()]
    if all_null_cols:
        X = X.drop(columns=all_null_cols)
    X = X.fillna(X.median(numeric_only=True)).fillna(0.0).astype("float64")
    ...
    for name, model in models.items():
        ...
        # --- SHAP ------------------------------------------------------------
        try:
            # background = clean float64 AND numpy array
            bg_pd = X_tr.sample(min(200, len(X_tr)), random_state=42).astype("float64")
            ...
            # handle list-return (e.g., multiclass APIs)
            shap_vals = np.asarray(shap_raw[0], dtype=np.float64) if isinstance(shap_raw, list) \
