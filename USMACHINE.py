...
with tabs[3]:
    ...
    # keep only rows whose State code is in LOWER48_ABBR
    gdf48 = (
        gdf[gdf["State"].isin(LOWER48_ABBR)]     # drop AK, HI, PR
          .dropna(subset=["State"])              # drop lake polygons (no state)
          .copy()
    )

    # helper → white background, black text
    def _white(fig):
        fig.update_layout(
            paper_bgcolor="white",
            plot_bgcolor="white",
        ...
        if st.checkbox("Show ML-predicted LCOH for all states", value=False):
            df_sub = (
                df.query("Tech == @tech & Year == @year")
                  .set_index("State")
                  .loc[LOWER48_ABBR]              # keep same 48 codes
            )
            preds48 = _predict48(df_sub)

            ml_map_df = (
                gdf48.merge(preds48.rename("LCOH ($/kg)"),
    ...
    # ───────────────────────── BASELINE-TEA VIEW ─────────────────────────
        ...
        try:
            ...
            if df_year.empty:
                ...
                merged = (
                    gdf48.merge(df_year[["State", "LCOH ($/kg)"]],
                                on="State", how="left")
                          .dropna(subset=["LCOH ($/kg)"])
                )
