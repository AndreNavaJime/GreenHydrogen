...
# --- Save default values in session state ---
...
if "cap_val" not in st.session_state:
    st.session_state.cap_val = default_cap
if "eff_val" not in st.session_state:
    st.session_state.eff_val = default_eff
if "elec_val" not in st.session_state:
    st.session_state.elec_val = default_elec
...
cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max), st.session_state.cap_val, key="cap_val")
eff_input = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max), st.session_state.eff_val, key="eff_val")
elec = st.sidebar.slider("Electricity price ($/kWh)", 0.01, 0.30, st.session_state.elec_val, key="elec_val")

# --- Adjusted values (based on user-selected year + tech) ---
cap_adjusted = adjust_capex_by_year(tech, year, st.session_state.cap_val)
eff = get_efficiency_by_year(tech, year)


# ✅ Use dynamically adjusted values based on tech + year
cap_dynamic = adjust_capex_by_year(tech, year, st.session_state.cap_val)
eff_dynamic = get_efficiency_by_year(tech, year)
...
row = {
    ...
    "Electricity_$/kWh": st.session_state.elec_val,  # Keep user-adjustable if needed
...
with tabs[0]:
    ...
    with st.expander("View Scenario Inputs"):
        st.write({
            ...
            "Year": year,
            "State": state,
            "CAPEX ($/kW)": cap_adjusted,
            "Efficiency (kWh/kg)": eff,
            "Electricity Price ($/kWh)": elec,  # ✅ Make sure this comma is here
...
with st.expander("View Scenario Inputs"):
    st.write({
        ...
        "CAPEX ($/kW)": cap_adjusted,
        "Efficiency (kWh/kg)": eff,
        "Electricity Price ($/kWh)": st.session_state.elec_val,
        "Capacity Factor": row_input['CF'].values[0]
    })
