...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        # 1️⃣ Generate one consistent scenario across all states
        dynamic_rows = []
        cf_map = df.set_index("State")["CF"].to_dict()

        for st_name in sorted(df.State.unique()):
            cf_val = cf_map.get(st_name, df.CF.median())
            row = {
                "State": st_name,
                "CF": cf_val,
                "CAPEX_$/kW": cap_dynamic,
            ...
            for feat in results[model_name]["features"]:
                if feat.startswith("Year_"):
                    row[feat] = 1 if feat == f"Year_{year}" else 0
            dynamic_rows.append(row)

        df_states = pd.DataFrame(dynamic_rows)

        # 2️⃣ Align and predict once for all
        df_features = pd.get_dummies(df_states.drop(columns=["State"]))
        for col in results[model_name]["features"]:
            df_features.setdefault(col, 0)
        df_features = df_features[results[model_name]["features"]]

        preds = best.predict(df_features)
        df_states["LCOH ($/kg)"] = np.clip(preds, 0, None).round(2)

        # Display map based on this single scenario
        merged = gdf.merge(df_states[["State", "LCOH ($/kg)"]], on="State", how="left")
        fig_map = px.choropleth(
            ...
            color_continuous_scale="Viridis",
            template="plotly_dark",
            title=f"Predicted LCOH by State ({year}, {tech}, {model_name})"
        )
        ...
        try:
            ...
            df_year = df_tech[df_tech["Year"] == year].rename(columns={lcoh_col: "LCOH ($/kg)"})
            ...
            st.error(f"Missing data: {file_name}")

    else:
        st.info("🌐 Map is available only in ML Model or True Equation mode.")
