...
# ──────────────────────────────────────────────────────────────────────────
#  TAB 6 – Download Center  (one-pager per state, no gaps, no repeats)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    import io
    try:
        from PyPDF2 import PdfReader, PdfWriter    # one import, one place
    ...
    # ── Residual plot for the scenario PDF (optional) ────────────────────
    ...
    # ── Ensure the data has one row per state ────────────────────────────
    df_sub = (df.query("Tech == @tech and Year == @year")
                .drop_duplicates(subset="State")
                .set_index("State"))
    if len(df_sub) != 48:
        missing = sorted(set(lower48) - set(df_sub.index))
        extra   = sorted(set(df_sub.index) - set(lower48))
        msg = []
        if missing: msg.append("missing: " + ", ".join(missing))
        if extra:   msg.append("unexpected: " + ", ".join(extra))
        st.error("Data table must cover all 48 states (" + "; ".join(msg) + ")")
        st.stop()

    # ── Vectorised prediction (cached) ───────────────────────────────────
