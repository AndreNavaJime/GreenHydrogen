...
with tabs[6]:
    ...
    # ---------- 1) One-page PDF for the current scenario ------------------
    ...
    # ---------- 2) Multi-file ZIP (current state, extras) -----------------
    ...
    # ---------- 3) One-page PDF for EVERY state → ZIP ---------------------
        ...
        st.info("Building one-page PDFs for every state …")
        all_zip = io.BytesIO()
        with zipfile.ZipFile(all_zip, "w") as z:
            for st_name in sorted(df.State.unique()):
                ...
                # deterministic LCOH with current sliders
                crf = 0.08 * (1 + 0.08) ** 20 / ((1 + 0.08) ** 20 - 1)
    ...
    # ---------- 4) NEW: Single multi-page PDF for all states --------------
    else:  # "All 48 States (Single PDF)"
        st.info("Compiling a single PDF that collates all 48 states …")
        from PyPDF2 import PdfReader, PdfWriter     # lightweight dependency
        writer = PdfWriter()
