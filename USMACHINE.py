...
with tabs[2]:
    ...
    # ── 0)  Kaleido availability & Plotly config ─────────────────────────
    try:
        import kaleido                      # noqa: F401 – presence test only
        _KALEIDO_OK = True
    except ModuleNotFoundError:
        _KALEIDO_OK = False

    _cfg = {                                # hide Plotly’s own toImage button
        "displaylogo": False,
        "modeBarButtonsToRemove": ["toImage"],
    }

    # ── local “white PNG” helper – only if Kaleido is present ────────────
    import copy, plotly.io as pio

    def export_png(fig, scale=2):
        """
        Return high-res PNG bytes with a white canvas.
        Raises RuntimeError if Kaleido is not available.
        """
        if not _KALEIDO_OK:
        ...
        return pio.to_image(f, format="png", scale=scale)
    ...
    if ml_mode and "best" in globals():
        ...
        if fi is not None:
            ...
            fig_bar.update_layout(xaxis_title="Importance", yaxis_title="Feature")

            st.plotly_chart(fig_bar, use_container_width=True, config=_cfg)

            # — custom download (only if Kaleido works) —
            if _KALEIDO_OK:
                bar_png = export_png(fig_bar)
                st.download_button(
                    "Download PNG (white canvas)",
                    bar_png,
                    file_name=f"feature_importance_{model_name}.png",
                    mime="image/png",
                )
        else:
    ...
    if ml_mode and "best" in globals():
        ...
        if shap_vals is not None and explainer is not None:
            ...
            plt.figure()
            shap.summary_plot(
                shap_vals_plot, X_train_plot,
                feature_names=pretty_names, show=False
            )
    ...
    if _KALEIDO_OK:
        hm_png = export_png(fig_hm)
        st.download_button(
            "Download heat-map PNG (white canvas)",
            hm_png,
            file_name="lcoh_trend_heatmap.png",
            mime="image/png",
        )
...
with tabs[3]:
    ...
    # safe PNG helper  (returns None on any export failure)
    import copy, plotly.io as pio
    def safe_png(fig, scale=2):
        try:
            f = copy.deepcopy(fig)
            ...
            return None   # Kaleido / Chrome not available
    ...
    if calculation_mode == "ML Model":
        ...
        fig_sel = px.choropleth(
            ...
            hover_name="State", hover_data={}, template="plotly_dark",
        ).update_traces(
        ...
        png_sel = safe_png(fig_sel)
        if png_sel is not None:
            st.download_button(
                "Download map PNG (white, no title)",
                png_sel,
                file_name=f"{state}_{tech}_{year}_highlight_map.png",
                mime="image/png",
            )
        else:
            st.info("PNG export unavailable (Kaleido not found).")

        # (B) optional 48-state choropleth
        show_full = st.checkbox(
            "Show ML-predicted LCOH for all states",
            value=False,
            help="Toggle a choropleth with 48-state predictions.",
        )
        ...
        if show_full:
            ...
            fig_ml = px.choropleth(
                ml_map_df, geojson=ml_map_df.geometry, locations=ml_map_df.index,
                color="LCOH ($/kg)", color_continuous_scale="Viridis",
                range_color=(2, 14),
                hover_name="State", hover_data={"LCOH ($/kg)":":.2f"},
                template="plotly_dark",
                title=f"{tech} – ML-Predicted LCOH ({year})",
            ).update_traces(
                marker_line_color="white",
            ...
            png_ml = safe_png(fig_ml)
            if png_ml is not None:
                st.download_button(
                    "Download map PNG (white, no title)",
                    png_ml,
                    file_name=f"{tech}_{year}_ML_choropleth.png",
                    mime="image/png",
                )
    ...
    # ───────────────────────── BASELINE-TEA VIEW ─────────────────────────
        ...
        try:
            ...
            if df_year.empty:
                ...
                png_te = safe_png(fig_te)
                if png_te is not None:
                    st.download_button(
                        "Download map PNG (white, no title)",
                        png_te,
                        file_name=f"{tech}_{year}_Baseline_choropleth.png",
                        mime="image/png",
                    )
