...
# ──────────────────────────────────────────────────────────────────────────
#  TAB 6  –  Download Center  (fast & duplicate-proof)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    st.subheader("Download Center")

    # ───────── Residual plot (cached) ─────────
    ...
    def _pdf_cache(state_dict):
        ...
        def _build(st_name):
            ...
            # patch the global header variable for generate_pdf
            global state
            prev, state = state, st_name
            pdf_bytes = generate_pdf(row, pred_val, fig_path, full=False)\
                          .output(dest="S").encode("latin1")
            state = prev
            return st_name, pdf_bytes

        with ThreadPoolExecutor(max_workers=8) as ex:
            return dict(ex.map(_build, lower48))
