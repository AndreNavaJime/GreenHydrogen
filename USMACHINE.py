...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        # Predict only for the selected state, then map that value across the map.
        # Create input row for the selected state:
        cf_val = df.loc[df.State == state, "CF"].iloc[0]
        row = {
            "State": state,
            "CF": cf_val,
            "CAPEX_$/kW": cap_dynamic,
        ...
        df_features = pd.get_dummies(df_sel.drop(columns=["State"]))
        for col in results[model_name]["features"]:
            if col not in df_features.columns:
                df_features[col] = 0
        df_features = df_features[results[model_name]["features"]]

        raw_pred = best.predict(df_features)[0]
        pred_val = float(max(raw_pred, 0))
        df_map = pd.DataFrame({"State": [state], "LCOH ($/kg)": [round(pred_val, 2)]})

        merged = gdf.merge(df_map, on="State", how="left")
        ...
        fig_map.update_coloraxes(colorbar_tickformat=".2f", showscale=False)
