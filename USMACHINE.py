...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        # Build consistent scenario rows for every state
        dynamic = []
        cf_map = df.set_index("State")["CF"].to_dict()

        for st_name in sorted(df.State.unique()):
            cf = cf_map.get(st_name, df.CF.median())
            row = {
                "State": st_name,
                "CF": cf,
                "CAPEX_$/kW": cap_dynamic,
        ...
        df_states = pd.DataFrame(dynamic)
        df_features = pd.get_dummies(df_states.drop(columns=["State"]))
        missing = set(results[model_name]["features"]) - set(df_features.columns)
        for c in missing:
            df_features[c] = 0
        ...
        # Merge for mapping
        merged = gdf.merge(df_states[["State", "LCOH ($/kg)"]], on="State")
        ...
        try:
            ...
            if df_year.empty:
                ...
                df_year = df_year.rename(columns={lcoh_col: "LCOH ($/kg)"})
                df_year["LCOH ($/kg)"] = df_year["LCOH ($/kg)"].round(2)
                merged = gdf.merge(df_year[["State", "LCOH ($/kg)"]], on="State")
                fig_map = px.choropleth(
                    merged, geojson=merged.geometry,
                    locations=merged.index, color="LCOH ($/kg)",
                    hover_name="State", color_continuous_scale="Viridis",
            ...
            st.error(f"Missing CSV: {file_name}")
    else:
        st.info("🌐 Map available only for ML Model or True Equation.")
