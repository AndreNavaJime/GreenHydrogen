...
cap_min = tech_settings["cap_min"]
cap_max = tech_settings["cap_max"]
eff_min = tech_settings["eff_min"]
eff_max = tech_settings["eff_max"]
...
elec_default = float(df[df.State == state]["Electricity_$/kWh_fixed"].iloc[0])

cap_default = max(min(cap_default, cap_max), cap_min)
eff_default = max(min(eff_default, eff_max), eff_min)

cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max), int(cap_default), step=1, key="cap_val", disabled=auto_mode)
eff_input = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max), float(eff_default), step=0.1, key="eff_val", disabled=auto_mode)
elec = st.sidebar.slider("Electricity price ($/kWh)", 0.01, 0.30, value=float(elec_default), step=0.01, key="elec_val")
