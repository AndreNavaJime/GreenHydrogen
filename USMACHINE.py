...
import plotly.io as pio  # ← FIXED: required to use pio.templates

# ── Streamlit page & global style ─────────────────────────────────────────
st.set_page_config(
    layout="centered",
...
/* ────────────── Targeted overrides ────────────── */
/* Dark banner for “Scenario Simulator” */
.sim-header{
    background:#0E1117;
...
st.sidebar.markdown(
    '<div class="sim-header">🔧 Scenario Simulator</div>',
    unsafe_allow_html=True
)
...
with tabs[2]:
    ...
    if fi is not None:
        ...
        fig = px.bar(fi_s.values, y=fi_s.index, orientation="h",
                     ...
                     template="plotly_white", title=" Feature Importance")
    ...
    fig2 = px.imshow(pivot, color_continuous_scale="Viridis", template="plotly_white",
...
with tabs[4]:
    ...
    for st_name in sorted(df.State.unique()):
        try:
            ...
            elec_val = elec_cents / 100.0  # cents → dollars
        ...
        for c, e in zip(cap_values, eff_values):
            row = {
                ...
                "Electricity_$/kWh": round(elec_val, 4),  # higher precision for energy cost
    ...
    summary_stats = (
        ...
        .applymap(lambda x: float(f"{x:.2f}"))  # Force 2 decimals as floats
    )
