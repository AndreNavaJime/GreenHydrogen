...
with tabs[2]:
    ...
    # keep Plotly’s camera button; hide its logo
    _cfg = {"displaylogo": False}

    # helper → white canvas, BLACK text/ticks, no title
    def _white_canvas(fig):
        fig.update_layout(
            paper_bgcolor="white",
            plot_bgcolor="white",
            font_color="black",
            title_text="",                 # guarantee no stray title
        )
        # force tick-label colour too
    ...
    _white_canvas(fig_hm)                                   # ★ black ticks/text
...
with tabs[3]:
    ...
    # helper → white canvas, white geo frame, black text
    def _white_canvas(fig):
        fig.update_layout(
            paper_bgcolor="white",
            plot_bgcolor="white",
            font_color="black",
            title_text="",                    # wipe any stray title
        )
        fig.update_xaxes(color="black")
        fig.update_yaxes(color="black")
        fig.update_geos(bgcolor="white")      # ← kills the dark strip
        # colour-bar ticks/title in black
