...
param_source = st.sidebar.radio(
    ...
    horizontal=True,
    help=(
        "**DOE targets**: sliders reset to the official DOE trajectory whenever you change year/tech.  \n"
        "**Fix inputs across years**: once you tweak a slider, its value stays the same no matter which year or technology you select."
    )
)
...
# ► if in DOE mode, reset session_state so sliders snap back to DOE defaults
if apply_doe:
    st.session_state.cap        = cap_official
    st.session_state.eta        = eff_official
    st.session_state.elec_price = elec_official

# ► dynamic slider ranges include static bounds, DOE defaults & manual input
