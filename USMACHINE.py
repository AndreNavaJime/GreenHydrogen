...
with tabs[5]:
    ...
    if national_df.empty:
        ...
        # Optional: Matplotlib PNG download (also no Kaleido)
        import io
        import matplotlib.pyplot as plt
        ...
        ax.plot(
            national_trend["Year"],
            national_trend["Average LCOH ($/kg)"],
            marker="o", linewidth=1.8
        )
        fig.patch.set_facecolor("white")
        ax.set_facecolor("white")
        ax.set_xlabel("Year", color="black")
        ax.set_ylabel("LCOH ($/kg)", color="black")
        ...
        buf.seek(0)
        st.download_button(
            "⬇️ Download PNG (white background, Matplotlib)",
            buf.getvalue(),
            file_name=f"{fn_base}.png",
            mime="image/png",
        )
