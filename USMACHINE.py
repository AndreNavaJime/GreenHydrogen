...
with tabs[3]:
    ...
    _cfg = {"displaylogo": False}  # keep camera icon

    # ── 1)  Build a “contiguous US only” GeoDataFrame ───────────────────
    # Bounding box of CONUS (rough): lon −125…−66, lat 24…50  (deg)
    conus_bbox = gdf.cx[-125:-66, 24:50]        # spatial filter
    gdf_conus  = conus_bbox.dropna(subset=["State"]).copy()  # drop water polygons

    # ── 2)  Plot-styling helper  (white canvas, black text) ─────────────
    def _white(fig):
        fig.update_layout(
            paper_bgcolor="white", plot_bgcolor="white",
            font_color="black", title_text=""
        )
    ...
    if calculation_mode == "ML Model":
        ...
        # (A) single-state highlight
        map_df = gdf_conus.copy()
        ...
        fig_sel = (
            ...
            .update_traces(marker_line_color="white",
                           hovertemplate="<b>%{hovertext}</b><extra></extra>",
                           showlegend=False)
            .update_layout(geo=dict(scope="usa", visible=False),
        ...
        if st.checkbox("Show ML-predicted LCOH for all states", value=False):
            ...
            preds = _predict48(df_sub)

            ml_map_df = gdf_conus.merge(
                preds.rename("LCOH ($/kg)"),
            ...
            fig_ml = (
                ...
                .update_layout(geo=dict(scope="usa"),
                               margin=dict(l=0, r=0, t=0, b=0))
            )
            _white(fig_ml)
