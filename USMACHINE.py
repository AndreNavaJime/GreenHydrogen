...
local_csv = r"C:\Users\Navar\OneDrive\Documentos\USA\TECHNO-ECONOMIC SIMULATION"
shp_path = r"C:\Users\Navar\OneDrive\Documentos\USA\SHAPEFILE\tl_2022_us_state.shp"
base_path = local_csv if os.path.exists(local_csv) else "."
paths = {
...
gdf = gpd.read_file(shp_path)[["STUSPS", "NAME", "geometry"]]
gdf = gdf.rename(columns={"STUSPS":"StateAbbr","NAME":"State"})
...
# --- 3. Sidebar Controls with Electricity slider ---
...
pred = best_model.predict(pred_df)[0]
...
with tabs[3]:
    subset = df[df.Year==year].groupby("State")["LCOH_$kg"].mean().reset_index()
    merged = gdf.merge(subset, on="State", how="left")
...
with tabs[4]:
    st.markdown("Download predicted values & feature importance")
    out = pd.DataFrame({"True": y_test.values, "Pred": results[best_name]["preds"], "Error": y_test.values - results[best_name]["preds"]})
    st.download_button("Predictions CSV", out.to_csv(index=False), "lcoh_preds.csv")
    st.download_button("Feature Importance CSV", fi.to_csv(), "feature_importance.csv")
