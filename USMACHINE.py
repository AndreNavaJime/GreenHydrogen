...
if calculation_mode == "ML Model":
    ...
    auto_mode = st.sidebar.checkbox(
        ...
        value=True,
        help="Uses future CAPEX and efficiency estimates from the U.S. Department of Energy’s official projections."
    )
...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        ...
        df_inputs["LCOH ($/kg)"] = best.predict(df_pred).round(2)  # ⬅️ Round to 2 decimals
        ...
        try:
            ...
            if df_year.empty:
                ...
                df_year["LCOH ($/kg)"] = df_year["LCOH ($/kg)"].round(2)  # ⬅️ Round to 2 decimals
