...
def train_models(df):
    ...
    df["Year_val"] = df["Year"]  # ✅ Continuous feature for better extrapolation
    X = pd.get_dummies(
        df.drop(columns=["State", "LCOH_$kg", "Electricity_$/kWh_fixed", "Year"]),
        columns=["Tech"],
        drop_first=True
    )
    X["Year_val"] = df["Year_val"]  # ✅ add numeric year feature
    y = df["LCOH_$kg"]
...
cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max), int((cap_min + cap_max) / 2))
eff = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max), round((eff_min + eff_max) / 2, 1))
state_elec_default = df.loc[df.State == state, "Electricity_$/kWh_fixed"].values[0]
elec = st.sidebar.slider("Electricity price ($/kWh)", 0.01, 0.30, float(round(state_elec_default, 4)))

# --- Dynamic CAPEX interpolation (DOE-aligned until 2060) ---
def adjust_capex_by_year(tech: str, year: int, original_capex: float) -> float:
    targets = {
...
# --- Dynamic Efficiency interpolation (based on system energy targets) ---
def get_efficiency_by_year(tech: str, year: int) -> float:
    efficiency_targets = {
...
cap_adjusted = adjust_capex_by_year(tech, year, cap)
eff = get_efficiency_by_year(tech, year)

# Show the real adjusted values in the sidebar
st.sidebar.markdown(f"**Adjusted CAPEX**: `${cap_adjusted}`/kW")
st.sidebar.markdown(f"**Adjusted Efficiency**: `{eff}` kWh/kg")

# --- Scenario Prediction Input & Model Evaluation ---
row = {
    "CF": float(df.query("State==@state & Year==@year & Tech==@tech").CF.iloc[0]),
    "CAPEX_$/kW": cap_adjusted,
...
# --- Handle missing one-hot encoded year and tech dummies dynamically
for col in features:
...
def build_model_input(row_dict, features):
    ...
    for col in features:
        if col not in row_df.columns:
            row_df[col] = 0  # fill missing dummies with 0
...
# --- Single Scenario Prediction Setup (Tabs 0 and 1) ---
try:
    cf_val = float(df.query("State == @state & Year == @year & Tech == @tech").CF.iloc[0])
except (IndexError, ValueError):
    st.error("No data found for the selected state/year/tech combination.")
    st.stop()
...
with tabs[0]:
    st.markdown(f"## Single Scenario Prediction")
