...
with tabs[6]:
    ...
    # ---------- Helpers --------------------------------------------------
    # Static list of the 48 contiguous U.S. states
    lower48 = [
    ...
    # Subset df to this tech & year, then drop any duplicate states
    df_sub    = df.query("Tech == @tech and Year == @year")
    df_unique = df_sub.drop_duplicates(subset="State").set_index("State")

    # --- DEBUG: verify uniqueness & completeness ---
    st.write("▶️ Unique states in df_unique:", len(df_unique))
    st.write("▶️ States present:", df_unique.index.tolist())
    missing = set(lower48) - set(df_unique.index)
    st.write("🚩 Missing states:", sorted(missing))
    ...
    # ---------- 3) 48 one-page PDFs → ZIP --------------------------------
        ...
        with zipfile.ZipFile(zip_buf, "w") as z:
            for i, st_name in enumerate(lower48, start=1):
                ...
                if st_name not in df_unique.index:
                    st.warning(f"No data for {st_name}; skipping.")
                    continue
                row_fix = df_unique.loc[st_name]
