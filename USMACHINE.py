...
def train_models(df: pd.DataFrame):
    ...
    Cleans any bracketed/EXPN numeric strings before training and SHAP.
    """
    import re, shap
    from sklearn.model_selection import train_test_split, KFold, cross_val_score
    ...
    def _clean_numeric_df(Xin: pd.DataFrame) -> pd.DataFrame:
        """
        Strip bracketed numbers like '[4.144E0]', commas, and spaces in object
        cols; coerce everything to float64; drop all-NaN cols; fill NaNs.
        """
        Xout = Xin.copy()

        # Clean object-typed columns with robust regex
        obj_cols = [c for c in Xout.columns if Xout[c].dtype == "object"]
        if obj_cols:
            pat = re.compile(r'^\s*\[?\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*\]?\s*$')
            for c in obj_cols:
                s = (Xout[c].astype(str)
                              .str.replace(",", "", regex=False)
                              .str.strip())
                s = s.apply(lambda v: pat.sub(r"\1", v) if pat.match(v) else np.nan)
                Xout[c] = pd.to_numeric(s, errors="coerce")

        # Coerce all to numeric, remove ±inf → NaN
        Xout = (Xout.apply(pd.to_numeric, errors="coerce")
                    .replace([np.inf, -np.inf], np.nan))

        # Drop columns entirely NaN after cleaning
        null_all = [c for c in Xout.columns if Xout[c].isna().all()]
        if null_all:
            Xout = Xout.drop(columns=null_all)

        # Fill with medians then zeros; cast to float64
        Xout = Xout.fillna(Xout.median(numeric_only=True)).fillna(0.0).astype("float64")
        return Xout
    ...
    X = pd.get_dummies(
        ...
        columns=["Tech"], drop_first=True
    )
    X["Year_val_squared"] = df_p["Year_val"] ** 2
    y = pd.to_numeric(df_p["LCOH_$kg"], errors="coerce")  # ensure numeric target

    # 2) Force numeric (robust)
    X = _clean_numeric_df(X)

    # 3) Drop rows where y is non-finite
    mask = np.isfinite(y)
    X, y = X[mask], y[mask]
    ...
    # NumPy views for models/SHAP that prefer arrays
    ...
    models = {
        ...
        "XGBoost": XGBRegressor(n_estimators=100, n_jobs=-1, verbosity=0, random_state=42),
    ...
    for name, model in models.items():
        ...
        # ---- Fit & predict (XGBoost gets NumPy) ---------------------------
        if lname == "xgboost":
        ...
        # --- SHAP ----------------------------------------------------------
        try:
            # Fully numeric NumPy background for all explainers
            bg_np = X_tr_np[: min(200, len(X_tr_np))]

            if lname in {"linear", "ridge"}:
                explainer = shap.LinearExplainer(model, bg_np)
                shap_raw = explainer.shap_values(bg_np[:500])

            elif lname in {"xgboost", "lightgbm", "catboost", "randomforest", "gbregressor"}:
                explainer = shap.TreeExplainer(
                    model, data=bg_np, feature_perturbation="interventional"
                )
                shap_raw = explainer.shap_values(bg_np[:500])

            else:
                explainer = shap.KernelExplainer(model.predict, bg_np)
                shap_raw = explainer.shap_values(bg_np[:200], nsamples=100)
        ...
        # --- quick 5-fold CV ----------------------------------------------
        cv_X = X_tr_np if lname == "xgboost" else X_tr
        cv_r2 = cross_val_score(
            model, cv_X, y_tr,  # sklearn handles pandas/np seamlessly here
        ...
        results[name] = {
            ...
            "cv_r2": float(cv_r2),
            "preds": np.asarray(preds, dtype=float).tolist(),
