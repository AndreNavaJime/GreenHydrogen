...
with tabs[6]:
    ...
    try:
        ...
        st.error("PyPDF2 ≥ 3.0 is required. Run:\n"
    ...
    # Filter, deduplicate, and align DataFrame
    tmp = df.query("Tech == @tech and Year == @year")
    state_counts = tmp["State"].value_counts()
    duplicated_states = state_counts[state_counts > 1].index.tolist()
    if duplicated_states:
        st.error(f"❌ Duplicated states found: {', '.join(duplicated_states)}.\n"
                 "You must fix your data to only have one row per state for this Tech+Year.")
        st.stop()
    tmp = tmp.drop_duplicates(subset="State", keep="first").set_index("State")
    df_sub = tmp.reindex(lower48)

    # Check for missing data
    if df_sub.isnull().any(axis=None):
        missing = df_sub[df_sub.isnull().any(axis=1)].index.tolist()
        st.error(f"❌ Missing data for states: {', '.join(missing)}")
        st.stop()

    # LCOH prediction (same as before)
    @st.cache_resource(show_spinner=True)
    def _predict_all(df_sub):
        df_v = df_sub.copy()
        if ml_mode:
            X = pd.DataFrame(0, index=df_v.index,
            ...
            if tech_col in X.columns:
                X[tech_col] = 1
            X = sanitize(X, results[model_name]["features"])
        ...
        return pd.Series(preds, index=df_v.index)
    preds_by_state = _predict_all(df_sub)

    # Residual plot for scenario PDF
    ...
    def _scenario_plot():
        ...
        ax.plot(results[model_name]["y_true"], lw=2, label="True")
        ax.plot(results[model_name]["preds"], ls="--", label="Pred")
    ...
    # Build exactly one single-page PDF per state
    pdf_bytes_by_state = {}
    for st_name in lower48:
        row  = df_sub.loc[st_name]
        pred = float(preds_by_state[st_name])

        # patch header var used by generate_pdf()
        _orig = globals().get("state")
        globals()["state"] = st_name

        raw = generate_pdf(row, pred, None, full=False) \
                .output(dest="S").encode("latin1")

        # keep *first* sheet (empirically the real state page)
        reader = PdfReader(io.BytesIO(raw))
        writer = PdfWriter()
        writer.add_page(reader.pages[0])
        buf = io.BytesIO(); writer.write(buf); buf.seek(0)
        pdf_bytes_by_state[st_name] = buf.read()
    ...
    if len({hash(b) for b in pdf_bytes_by_state.values()}) != 48:
        st.error("Duplicate pages detected – export aborted. Check your data and the generate_pdf logic.")
        st.stop()
