...
with tabs[2]:
    fi_vals = getattr(best, "feature_importances_", None)
    if fi_vals is not None:
        fi = pd.Series(fi_vals, index=X_train.columns).sort_values()
        fig_bar = px.bar(
            x=fi.values,
            y=fi.index,
            orientation="h",
    ...
    pivot = (df.groupby(["Tech", "Year"])["LCOH_$kg"]
               .mean()
               .reset_index()
...
with tabs[3]:
    ...
    fig_map.update_coloraxes(colorbar_tickformat=".1f")
    fig_map.update_geos(
        scope="usa",
        showcoastlines=True,
...
with tabs[4]:
    ...
    df_out = pd.DataFrame({
        "True": y_test.values,
        "Predicted": results[best_model_name]["preds"],
