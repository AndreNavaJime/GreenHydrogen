...
# ──────────────────────────────────────────────────────────────────────────
#  TAB 6 – Download Center  (duplicate-proof, gap-proof, PdfReader always in scope)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    import io
    try:                                           # global PyPDF2 guard
        from PyPDF2 import PdfReader, PdfWriter
    except ModuleNotFoundError:
        st.error(
            "PyPDF2 ≥ 3.0 is required for the 48-state export.\n"
            "Run  pip install PyPDF2 --upgrade  and restart the app."
        )
        st.stop()

    st.subheader("Download Center")

    # ── Residual plot for the single-scenario PDF only ───────────────────
    ...
    # ── Canonical list of the 48 contiguous U.S. states ──────────────────
    lower48 = [
    ...
    # ── Require one data row per state before proceeding ─────────────────
    df_sub = (df.query("Tech == @tech and Year == @year")
                .drop_duplicates(subset="State")
                .set_index("State"))

    missing_rows = sorted(set(lower48) - set(df_sub.index))
    if missing_rows:
        st.error("❌ No data rows for: " + ", ".join(missing_rows))
        st.stop()

    # ── Vectorised LCOH prediction (ML or TEA) ───────────────────────────
    @st.cache_resource(show_spinner=False)
    ...
    from PyPDF2 import PdfReader, PdfWriter   # ensure symbols right here
    pdf_bytes_by_state = {}
    for st_name in lower48:
        row, pred_val = df_sub.loc[st_name], float(preds_by_state[st_name])

        # patch header var for generate_pdf()
        orig_state = globals().get("state", None)
        globals()["state"] = st_name

        raw = generate_pdf(row, pred_val, None, full=False)\
                .output(dest="S").encode("latin1")
        reader = PdfReader(io.BytesIO(raw))
        writer = PdfWriter()
        writer.add_page(reader.pages[-1])          # keep only the last sheet
        buf = io.BytesIO(); writer.write(buf); buf.seek(0)
        pdf_bytes_by_state[st_name] = buf.read()

        # restore
        if orig_state is not None:
            globals()["state"] = orig_state
        else:
            del globals()["state"]

    # extra duplicate guard
    if len({hash(b) for b in pdf_bytes_by_state.values()}) != 48:
        st.error("Duplicate pages detected – export aborted.")
        st.stop()
