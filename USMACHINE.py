...
# --- Layout Tabs ---
tabs = st.tabs(["Prediction", "Stats", "Visuals", "Map", "Download", "State Comparison"])
...
with tabs[3]:
    ...
    for st_name in sorted(df.State.unique()):
        try:
            ...
            continue  # Skip states with no data for this year/tech
        dynamic_rows.append({
            "Year": year,
            "CF": cf_val,
            "CAPEX_$/kW": cap,
...
with tabs[5]:
    ...
    summary_stats = df_inputs.groupby("State")["LCOH ($/kg)"].agg(["mean", "min", "max", "std"]).round(2)

    if not summary_stats.isna().values.all():
        try:
            st.dataframe(summary_stats.style.background_gradient(cmap="plasma"))
        except Exception as e:
            st.warning(f"Could not apply gradient styling: {e}")
            st.dataframe(summary_stats)  # Fallback to plain DataFrame
    else:
