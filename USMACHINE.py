...
with tabs[3]:
    ...
    # 1️⃣ Get scenario prediction (computed above in 'pred')
    if calculation_mode != "ML Model":
        st.info("Switch to **ML Model** mode to view this map.")
    else:
        # 'pred' already computed earlier (scenario prediction)
        lcoh = round(pred, 2)

        # 2️⃣ Prepare map DataFrame: full USA, highlight selected state
        map_df = gdf.copy()
        map_df["Highlight"] = (map_df["State"] == state)

        # 3️⃣ Plot with discrete color scale: cyan for the state, gray for others
        ...
        fig_map.update_layout(coloraxis_showscale=False)  # Hide color bar
