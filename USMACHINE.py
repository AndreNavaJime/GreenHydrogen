# --- Standard libs ---
import os, sys, time, io, zipfile, warnings, re
...
# Gradient boosters (keep; they’re in the new stack)
...
TEA_DIR_CANDIDATES = [
    os.path.join(PROJECT_ROOT, "TECHNO-ECONOMIC SIMULATION"),
    r"C:\Users\Navar\OneDrive\Documentos\USA\TECHNO-ECONOMIC SIMULATION",
    APP_DIR,
]
GEOJSON_URL = (
...
def resolve_tea_dir():
    for path in TEA_DIR_CANDIDATES:
...
def safe_filename_token(value):
    ...
    return token.strip("_") or "scenario"


def publication_plotly(fig, showgrid=False):
    fig.update_layout(
        template="plotly_white",
        paper_bgcolor="white",
        plot_bgcolor="white",
        font=dict(color="black", size=13),
        title=dict(font=dict(size=15, color="black"), x=0.02, xanchor="left"),
        margin=dict(l=50, r=20, t=35, b=45),
    )
    fig.update_xaxes(showgrid=showgrid, gridcolor="rgba(0,0,0,0.12)", zeroline=False)
    fig.update_yaxes(showgrid=showgrid, gridcolor="rgba(0,0,0,0.12)", zeroline=False)
...
def lcoh_display_label(mode):
    if mode == "ML Model":
        return f"Plant-gate LCOH ({LCOH_UNIT})"
    return f"Baseline TEA LCOH ({LCOH_UNIT})"
...
def load_electricity_prices():
    file_path = data_path("ElectricityUSA.xlsx")
    if not os.path.exists(file_path):
        st.error(f"Missing file: {file_path}. Please upload it to your working directory.")
        st.stop()

    df_elec = pd.read_excel(file_path)
    if df_elec.shape[1] < 2:
        st.error("ElectricityUSA.xlsx must contain State and electricity-price columns.")
        st.stop()
...
@st.cache_data(show_spinner=False)
def load_us_geometries(states_tuple):
    states = set(states_tuple)
    sources = []
    local_geojson = data_path("us-states.geojson")
    if os.path.exists(local_geojson):
        sources.append(local_geojson)
    sources.append(GEOJSON_URL)
    ...
    for source in sources:
        try:
            ...
            if "name" in gdf.columns and "State" not in gdf.columns:
                gdf = gdf.rename(columns={"name": "State"})
            if "State" not in gdf.columns:
                continue
            if gdf.crs is None:
                gdf = gdf.set_crs("EPSG:4326", allow_override=True)
            return gdf[gdf["State"].isin(states)].copy()
        except Exception as exc:
            last_error = exc

    st.warning(f"U.S. map geometry could not be loaded. Map tabs will be unavailable. Last error: {last_error}")
    return gpd.GeoDataFrame({"State": [], "geometry": []}, geometry="geometry", crs="EPSG:4326")


@st.cache_data
def load_data():
    # ---------- 1) TARGET tables ----------
    base = resolve_tea_dir()
    ...
    water = pd.read_csv(data_path("USWATER.csv")).rename(columns=lambda c: c.strip())
    ...
    land_raw = pd.read_csv(data_path("USLAND.csv")).rename(
    ...
    gdf = load_us_geometries(tuple(sorted(lower48)))
    ...
    for tech, filename in [
        ...
        try:
            ...
            # safety renames only if CSVs used alternate names
            rename_map = {
                "rank_pg_colname":   "rank_pg",
                "rank_low_colname":  "rank_low",
                "rank_med_colname":  "rank_med",
                "rank_high_colname": "rank_high",
                "flip_low_colname":  "flip_low",
                "flip_med_colname":  "flip_med",
                "flip_high_colname": "flip_high",
            }
...
def train_models(
    df: pd.DataFrame,
    *,
    # Keep the optimized 4-model default
    learners=("OLS", "Ridge", "XGBoost", "LightGBM"),
    ...
    # Profiles & SHAP throttle
    speed: str = "standard",        # "standard" | "fast" | "thorough"
    shap_mode: str = "off",         # "off" | "selected" | "all"
):
    """
    TRAIN MODELS (optimized 4-model stack)
    - Same outputs schema as before.
    - Models kept: OLS, Ridge, XGBoost, LightGBM.
    - Speed profiles tune only XGB/LGBM.
    - SHAP throttle: off | selected (XGB/LGBM) | all.
    ...
    # --- GroupKFold by State (or other groups_col), if available ---
    groups = None
    if groups_col in df_p.columns:
    ...
    # ---------- 3) Model zoo with speed profiles (XGB/LGBM only) ----------
    # Baseline (STANDARD) hyperparameters
    xgb_params  = dict(
        n_estimators=200, max_depth=6, learning_rate=0.07,
        subsample=0.9, colsample_bytree=0.9, reg_lambda=1.0, reg_alpha=0.0,
        n_jobs=-1, verbosity=0, tree_method="hist", max_bin=256,
        random_state=random_state
    )
    lgbm_params = dict(
        n_estimators=300, max_depth=8, num_leaves=31, learning_rate=0.07,
        subsample=0.9, colsample_bytree=0.9, n_jobs=-1, random_state=random_state
    )

    if speed == "fast":
        # ~30–50% faster; negligible accuracy loss
        xgb_params.update(n_estimators=150, max_depth=5, max_bin=128, learning_rate=0.08)
    ...
    # filter to requested learners
    models = {k: v for k, v in all_defs.items() if k in learners}
    if not models:
        st.sidebar.error("No valid learners available with current environment."); return {}, X
    ...
    for m_i, (name, proto) in enumerate(models.items(), start=1):
        ...
        # SHAP (optional; small background; boosters by default for "selected")
        ...
        if shap_mode != "off":
            try:
                ...
                want = (
                    (shap_mode == "all") or
                    (shap_mode == "selected" and name in ("XGBoost", "LightGBM"))
                )
                if want:
                    try:
...
st.sidebar.caption(
    "ML Model emulates deterministic TEA plant-gate LCOH; it is not a market-price forecast. "
    "Baseline TEA evaluates the analytical TEA equation for the selected scenario."
)
...
def _predict48(df_v: pd.DataFrame, cache_key: str):
    ...
    if ml_mode:
        ...
        # carry through retained state/resource/infrastructure predictors
        for feat in feats:
            if feat in df_v.columns:
                X[feat] = pd.to_numeric(df_v[feat], errors="coerce")

        # user-controlled core scenario features
        if "CF" in feats:
            X["CF"] = pd.to_numeric(df_v["CF"], errors="coerce").fillna(0.0)
        if "CAPEX_$/kW" in feats:
            X["CAPEX_$/kW"] = cap_dynamic
        if "Efficiency_kWh_kg" in feats:
        ...
        for col, default in {
            ...
            if col in feats:
                X[col] = _series_or_default(df_v, col, default)

        # time features
        if "Year_val" in feats:
            X["Year_val"] = year
        if "Year_val_squared" in feats:
...
# Track DOE mode transitions
if "last_param_source" not in st.session_state:
    st.session_state.last_param_source = param_source

# ► if DOE mode and (year/tech changed OR just switched FROM fixed mode), reset to defaults
if apply_doe and (
    st.session_state.last_year != year
    or st.session_state.last_tech != tech
    or st.session_state.last_param_source != param_source  # ← NEW: detect mode switch
):
    st.session_state.cap        = cap_official
    st.session_state.eta        = eff_official
    st.session_state.elec_price = safe_elec_default

# ► Remember the current parameter source for next run
st.session_state.last_param_source = param_source
...
if ml_mode:
    for col in results[model_name]["features"]:
        if col not in row and col in row_base.index:
            row[col] = _get_num_from_series(row_base, col, 0.0)
...
def generate_pdf(row, pred, fig_path=None, full=False):
    ...
    pdf.multi_cell(
        ...
        f"{lcoh_display_label(calculation_mode)}: ${pred:.2f}/kg\n"
        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
        "Note: ML mode is a deterministic TEA surrogate/emulator, not a market-price forecast."
    )
...
with tabs[0]:
    ...
    st.metric(lcoh_display_label(calculation_mode), f"${pred_val:.2f}/kg")
    if ml_mode:
        st.caption("Surrogate/emulator output for deterministic TEA plant-gate LCOH, not a market-price forecast.")
    else:
        st.caption("Analytical Baseline TEA scenario; adders are included when present in the input table.")
...
with tabs[1]:
    ...
    if ml_mode and isinstance(results, dict) and len(results) > 0:
        # Build tidy metrics table from results dict (robust to missing keys)
        rows = []
        for name, slot in results.items():
            rows.append({
                "Model": name,
                "R² (OOF)":        _safe_float(slot.get("oof_r2")),
                "RMSE (OOF)":      _safe_float(slot.get("oof_rmse")),
                "MAE (OOF)":       _safe_float(slot.get("oof_mae")),
                "R² (Hold-out)":   _safe_float(slot.get("holdout_r2")),
                "RMSE (Hold-out)": _safe_float(slot.get("holdout_rmse")),
                "MAE (Hold-out)":  _safe_float(slot.get("holdout_mae")),
            })

        stats_df = pd.DataFrame(rows)
        if stats_df.empty:
            st.info("No model metrics available yet. Train a model in the Prediction tab.")
            st.stop()

        stats_df = stats_df.set_index("Model")

        # Sort by OOF R² (desc) while keeping NaNs at the bottom
        if "R² (OOF)" in stats_df.columns:
            stats_df = stats_df.sort_values(by="R² (OOF)", ascending=False, na_position="last")

        # Model picker — prefer previously selected, else best by OOF R²
        model_options = list(stats_df.index)
        if not model_options:
            st.info("No trained models to display.")
            st.stop()

        # If external model_name exists and valid, use it; else default to first row
        if ('model_name' in globals()) and (model_name in model_options):
            default_idx = model_options.index(model_name)
        else:
            default_idx = 0
        model_name = st.selectbox("Select model", model_options, index=default_idx)

        # Show metrics table (rounded)
        show_df = stats_df.copy().applymap(lambda v: np.round(v, 3) if pd.notna(v) else np.nan)
        ...
        # Highlight selected model’s metrics (robust to NaNs)
        ...
        oof_str = f"R²={_fmt(sel.get('R² (OOF)'))}, RMSE={_fmt(sel.get('RMSE (OOF)'))}, MAE={_fmt(sel.get('MAE (OOF)'))}"
        holdout_avail = (
            np.isfinite(_safe_float(sel.get("R² (Hold-out)"))) or
            np.isfinite(_safe_float(sel.get("RMSE (Hold-out)"))) or
            np.isfinite(_safe_float(sel.get("MAE (Hold-out)")))
        )
        if holdout_avail:
            holdout_str = (
                f" | Hold-out: R²={_fmt(sel.get('R² (Hold-out)'))}, "
                f"RMSE={_fmt(sel.get('RMSE (Hold-out)'))}, "
                f"MAE={_fmt(sel.get('MAE (Hold-out)'))}"
            )
        else:
            holdout_str = " | Hold-out: n/a"

        st.success(f"**{model_name}** — {oof_str}{holdout_str}")
        st.caption("OOF = out-of-fold predictions from cross-validation. Metrics evaluate emulation of deterministic TEA plant-gate LCOH.")
...
# ─────────────────────────────────────────────────────────────────────────
with tabs[2]:
    st.markdown("## Model Diagnostics & Trends")
    _cfg = {"displaylogo": False}

    # needed for the optional random 80/20 hold-out below
    from sklearn.base import clone
    ...
    if ml_mode and results and (model_name in results):
        ...
        # If it’s a Pipeline with TTR, peel to the raw estimator
        if hasattr(inf_model, "named_steps") and "ttr" in inf_model.named_steps:
            ttr = inf_model.named_steps["ttr"]
            final_est = getattr(ttr, "regressor_", ttr)
        else:
            final_est = inf_model
        ...
        if fi is not None:
            ...
            fig_bar = px.bar(x=fi_show.values, y=y_labels, orientation="h",
                             ...
                             labels={"x": "Model diagnostic importance", "y": "Feature"}, template="none")
            ...
            st.caption("Feature importance is a model diagnostic for the TEA surrogate, not causal attribution.")
        ...
        if (shap_vals is not None) and (explainer is not None) and (X_train is not None) and (len(X_train) > 0):
            ...
            try:
                ...
                fig_bar2 = px.bar(df_bar, x="Mean |SHAP|", y="Feature", orientation="h",
                                  template="none", labels={"Mean |SHAP|": "Mean absolute SHAP value", "Feature": ""})
        ...
        fig_par.update_layout(
            title=f"Parity diagnostic ({model_name}, {note})",
            xaxis_title=f"Deterministic TEA plant-gate LCOH ({LCOH_UNIT})",
            yaxis_title=f"ML-emulated plant-gate LCOH ({LCOH_UNIT})",
        ...
        st.caption(f"Diagnostic fit: yhat = {intercept:.3f} + {slope:.3f} x y; R2 = {r2_parity:.3f}.")
        ...
        fig_res.update_layout(title="Residual diagnostic",
                              xaxis_title=f"ML-emulated plant-gate LCOH ({LCOH_UNIT})",
                              yaxis_title=f"Residual ({LCOH_UNIT})",
                              margin=dict(l=40, r=10, t=10, b=40))
...
with tabs[3]:
    ...
    # Helper: robustly find an LCOH column in a DataFrame
    def _find_lcoh_col(df: pd.DataFrame) -> str | None:
        # 1) strongest signal — any "plantgate"
        for c in df.columns:
            if "plantgate" in c.replace(" ", "").lower():
                return c
        # 2) any column that mentions 'lcoh' (case-insensitive)
        cands = [c for c in df.columns if "lcoh" in c.lower()]
    ...
    maps_available = (not gdf48.empty) and ("geometry" in gdf48.columns)
    ...
    if calculation_mode == "ML Model":
        ...
        st.markdown(f"### {state}: ML plant-gate LCOH = ${pred:.2f}/kg")
        st.caption("Map values are ML-emulated deterministic TEA plant-gate LCOH.")

        if not maps_available:
            st.warning("Map geometry is unavailable. Scenario metrics and tables remain available.")
        ...
        if st.checkbox("Show ML-predicted LCOH for all states", value=False):
            ...
            if df_sub.empty:
                ...
                ml_map_df = (
                    gdf48.merge(
                        preds48.rename(f"Plant-gate LCOH ({LCOH_UNIT})"),
                        left_on="State", right_index=True, how="left",
                    )
                    .dropna(subset=[f"Plant-gate LCOH ({LCOH_UNIT})"])
                )
                ...
                if ml_map_df.empty:
                    ...
                    fig_ml = (
                        px.choropleth(
                            ...
                            color=f"Plant-gate LCOH ({LCOH_UNIT})",
                            color_continuous_scale="Viridis",
                            range_color=(2, 14),
                            hover_name="State",
                            hover_data={f"Plant-gate LCOH ({LCOH_UNIT})":":.2f"},
                            template="plotly_dark",
                        )
                        .update_traces(
                            marker_line_color="white",
                            hovertemplate="<b>%{hovertext}</b>"
                                          "<br>Plant-gate LCOH: %{z:.2f} US$ kg^-1<extra></extra>",
                        )
                        .update_layout(
                            geo=dict(scope="usa"),
                            margin=dict(l=0, r=0, t=0, b=0),
    ...
    # ───────────────────────── BASELINE-TEA VIEW ─────────────────────────
        ...
        csv_file = os.path.join(resolve_tea_dir(), files[tech])

        try:
            df_tech = pd.read_csv(csv_file)

            # robust LCOH column detection
            lcoh_col = _find_lcoh_col(df_tech)
            if not lcoh_col:
                st.error(f"No LCOH column found in `{csv_file}`. Columns: {list(df_tech.columns)}")
                st.stop()

            # standardize, coerce numeric, round
            lcoh_col_clean = f"Plant-gate LCOH ({LCOH_UNIT})"
            df_tech = df_tech.rename(columns={lcoh_col: lcoh_col_clean})
            df_tech[lcoh_col_clean] = pd.to_numeric(df_tech[lcoh_col_clean], errors="coerce")

            # filter by year after coercion; then round (prevents KeyError if column types were mixed)
            df_year = df_tech[df_tech["Year"] == year].copy()
            if df_year.empty:
                st.warning("No data for that year/tech.")
            else:
                df_year[lcoh_col_clean] = df_year[lcoh_col_clean].round(2)

                merged = (
                    gdf48.merge(
                        df_year[["State", lcoh_col_clean]],
                        on="State", how="left"
                    )
                    .dropna(subset=[lcoh_col_clean])
                )
...
# ─────────────────────────────────────────────────────────────────────────
with tabs[4]:
    st.subheader("State-wise LCOH summary from local slider variations")
    st.caption("This table summarizes nearby CAPEX and efficiency scenarios for the selected mode; it is not the Monte Carlo uncertainty analysis.")
    ...
    _best = None
    if ml_mode and isinstance(results, dict) and len(results) > 0:
        # choose a valid model if external model_name is missing
        ...
        features = rslot.get("features", []) or []
    ...
    def _elec_fallback(subset_df):
        ...
        # last resort: current global slider or 0.10 $/kWh
        return _n(elec, 0.10)

    if "Electricity_$/kWh" not in df.columns:
        st.warning("Electricity_$/kWh column is missing; using slider fallback for all states.")

    # Pre-compute which optional engineered features exist in the trained model
    has_year_val = "Year_val" in features
    has_year_sq  = "Year_val_squared" in features
    tech_ohe_cols = [c for c in features if c.startswith("Tech_")]

    for st_name in sorted(df.State.dropna().unique()):
        ...
        for c, e in product(cap_values, eff_values):
            ...
            if ml_mode and _best is not None and features:
                # Add optional temporal + tech one-hots only if the trained model expects them
                if has_year_val:
                    row_sim["Year_val"] = float(year)
                if has_year_sq:
                    row_sim["Year_val_squared"] = float(year) ** 2

                # Set any expected Tech_* one-hot columns; all 0 except the active tech if present
                for col in tech_ohe_cols:
                    # col format: "Tech_PEM", "Tech_SOEC", ... (drop_first=True)
                    expected = col.split("Tech_", 1)[-1]
                    row_sim[col] = 1 if (str(tech) == expected) else 0
                ...
                # Deterministic TEA fallback (kept simple and consistent with main TEA)
                # CRF with 8% real discount and 20-year life
                disc = 0.08; N = 20
                crf  = disc * (1 + disc) ** N / ((1 + disc) ** N - 1)

                # Annual H2 (kg) per kW: CF * 8760 / SEC
                kgH2_per_kWyr = cf_val * 8760.0 / e
                if not np.isfinite(kgH2_per_kWyr) or kgH2_per_kWyr <= 0:
                    continue

                # Levelized terms per kg – match true_lcoh() elsewhere
                capex_term = (c * crf) / kgH2_per_kWyr
                opex_term  = (0.05 * c * (c / cap_ref)) / kgH2_per_kWyr
                elec_term  = elec_val * e  # $/kWh * kWh/kg

                val = capex_term + opex_term + elec_term + wc + cs + tc + sc
    ...
    if not variations:
        ...
        summary_stats = (
            df_var.groupby("State", as_index=True)["LCOH ($/kg)"]
...
with tabs[5]:
    ...
    if national_df.empty:
        ...
        if calculation_mode == "ML Model":
            ...
            def _mean_safe(subset, col, default=np.nan):
                if col in subset.columns:
                    v = pd.to_numeric(subset[col], errors="coerce").mean()
            ...
            for y in years:
                ...
                elec_val = _mean_safe(subset, "Electricity_$/kWh", np.nan)
                cf_val   = _mean_safe(subset, "CF", np.nan)

                wc = _mean_safe(subset, "Water_$kg", 0.0)
                cs = _mean_safe(subset, "CO2_$kg", 0.0)
                tc = _mean_safe(subset, "Transport_$kg", 0.0)
                sc = _mean_safe(subset, "Storage_$kg", 0.0)

                # If key inputs are missing, skip this year
                if (not np.isfinite(eff_val)) or eff_val <= 0 or (not np.isfinite(cf_val)) or cf_val <= 0:
                    continue
                if not np.isfinite(elec_val):
                    # fallback to global slider if needed
                    elec_val = float(st.session_state.get("elec_price", 0.10))
        ...
        if not national_trend.empty:
            ...
            if "Year" in national_trend.columns and not national_trend.empty:
...
with tabs[7]:
    ...
    def get_export_predictions():
        ...
        if df_sub.empty:
            return pd.Series(dtype="float64"), "No data available for the selected Technology and Year."
    ...
    # (gdf may already be lower-48 filtered in Tab 3; handle both cases)
        valid_states = set(gdf["State"]) if ("State" in gdf.columns and not gdf.empty) else set(df_sub.index)
        df_sub = df_sub.loc[df_sub.index.intersection(valid_states)].sort_index()

        if df_sub.empty:
            return pd.Series(dtype="float64"), "No states are available for the selected configuration."

    # ✅ use cache_key so cache refreshes when UI changes
        try:
            preds = _predict48(df_sub, cache_key)   # series indexed by state names
        except Exception as exc:
            return pd.Series(dtype="float64"), f"Could not compute predictions for export: {exc}"

    # Drop any states without a numeric prediction
        preds = preds[preds.replace([np.inf, -np.inf], np.nan).notna()]
        if preds.empty:
            return pd.Series(dtype="float64"), "Predictions are empty for the selected configuration."
        return preds, None
    ...
    def make_summary_page() -> bytes:
        ...
        pdf.multi_cell(
            ...
            ascii_safe(
                ...
                f"Target label: {lcoh_display_label(calculation_mode)}\n"
                f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
                "Assumption note: ML mode is a deterministic TEA surrogate/emulator, not a market-price forecast."
            )
        )
    ...
    def make_state_page(st_name: str, pred_val: float) -> bytes:
        pdf = FPDF(); pdf.add_page()
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, ascii_safe(f"{st_name} - {lcoh_display_label(calculation_mode)}"), ln=True)
        pdf.set_font("Arial", "", 12)
        pdf.cell(0, 10, ascii_safe(f"LCOH: ${pred_val:,.2f}/kg"), ln=True)
        pdf.ln(4)
        pdf.set_font("Arial", "I", 9)
        pdf.multi_cell(0, 6, ascii_safe("Generated from the TEA-linked Streamlit surrogate/dashboard."))
        return pdf.output(dest="S").encode("latin1")
    ...
    def build_model_metrics_df():
        rows = []
        if isinstance(results, dict):
            for name, slot in results.items():
                rows.append({
                    "Model": name,
                    "R2_OOF": _num_or_nan(slot.get("oof_r2")),
                    "RMSE_OOF": _num_or_nan(slot.get("oof_rmse")),
                    "MAE_OOF": _num_or_nan(slot.get("oof_mae")),
                    "R2_holdout": _num_or_nan(slot.get("holdout_r2")),
                    "RMSE_holdout": _num_or_nan(slot.get("holdout_rmse")),
                    "MAE_holdout": _num_or_nan(slot.get("holdout_mae")),
                })
        return pd.DataFrame(rows)

    def build_model_ready_dataset_df():
        meta_cols = [c for c in ["State", "Year", "Tech", "LCOH_$kg"] if c in df.columns]
        if X_train is not None and len(X_train) > 0:
            meta = df.loc[X_train.index, meta_cols].reset_index(drop=True)
            features = X_train.reset_index(drop=True)
            return pd.concat([meta, features], axis=1)
        return df.copy()

    def _extract_final_estimator(slot):
        inf_model = slot.get("inference_model", slot.get("model"))
        if hasattr(inf_model, "named_steps") and "ttr" in inf_model.named_steps:
            ttr = inf_model.named_steps["ttr"]
            return getattr(ttr, "regressor_", ttr)
        return inf_model
    ...
    def build_feature_importance_df():
        ...
        if not isinstance(results, dict):
            return pd.DataFrame(rows)
        for model, slot in results.items():
            feats = slot.get("features", []) or []
            est = _extract_final_estimator(slot)
            vals = getattr(est, "feature_importances_", None)
            if vals is None and hasattr(est, "coef_"):
                vals = np.ravel(np.abs(getattr(est, "coef_")))
            if vals is None:
                continue
            for feat, val in zip(feats, np.ravel(vals)):
                rows.append({"Model": model, "Feature": feat, "Importance": _num_or_nan(val)})
        return pd.DataFrame(rows)
    ...
    def build_predictions_df():
        ...
        meta = None
        if X_train is not None and len(X_train) > 0:
            meta_cols = [c for c in ["State", "Year", "Tech"] if c in df.columns]
            meta = df.loc[X_train.index, meta_cols].reset_index(drop=True)
        for model, slot in results.items():
            yt = slot.get("y_true_oof")
            yp = slot.get("y_pred_oof")
            if yt is not None and yp is not None:
                pred_df = pd.DataFrame({"True": yt, "Predicted": yp})
                pred_df["Error"] = pred_df["Predicted"] - pred_df["True"]
                pred_df["Model"] = model
                pred_df["Split"] = "OOF_GroupKFold_by_state"
                if meta is not None and len(meta) == len(pred_df):
                    pred_df = pd.concat([meta.copy(), pred_df], axis=1)
                rows.append(pred_df)
            ht = slot.get("y_true_holdout")
            hp = slot.get("y_pred_holdout")
            if ht is not None and hp is not None:
                hold_df = pd.DataFrame({"True": ht, "Predicted": hp})
                hold_df["Error"] = hold_df["Predicted"] - hold_df["True"]
                hold_df["Model"] = model
                hold_df["Split"] = "random_80_20_holdout"
                rows.append(hold_df)
        return pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()

    def researcher_readme_text():
        return (
            "Complete researcher dataset export\\n"
            f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}\\n\\n"
            "Scientific scope:\\n"
            "- The ML workflow is a deterministic TEA-linked surrogate/emulator for plant-gate LCOH.\\n"
            "- It is not a market-price forecast and should not be interpreted as empirical price prediction.\\n"
            "- Baseline TEA mode is separate and evaluates the analytical TEA equation for the selected scenario.\\n"
            "- No TEA assumptions, model target, validation procedure, or scenario logic are changed by this export.\\n\\n"
            "Included files when available:\\n"
            "- model_ready_dataset.csv: target metadata plus model-ready numeric features.\\n"
            "- model_metrics.csv: OOF and hold-out validation metrics for trained surrogate models.\\n"
            "- feature_importance.csv: model diagnostic importances, not causal effects.\\n"
            "- predictions.csv: OOF and hold-out prediction diagnostics when available.\\n"
            "- merged_state_inputs.csv: merged deterministic TEA and state/resource input table used by the app.\\n"
            "- scenario_outputs.csv: local slider-variation outputs if generated in this app run.\\n"
        )

    def build_researcher_dataset_zip():
        files = {
            "model_ready_dataset.csv": build_model_ready_dataset_df(),
            "model_metrics.csv": build_model_metrics_df(),
            "feature_importance.csv": build_feature_importance_df(),
            "predictions.csv": build_predictions_df(),
            "merged_state_inputs.csv": df.copy(),
        }
        scenario_df = globals().get("df_var")
        if isinstance(scenario_df, pd.DataFrame) and not scenario_df.empty:
            files["scenario_outputs.csv"] = scenario_df.copy()

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            z.writestr("README_assumptions.txt", researcher_readme_text())
            for name, frame in files.items():
                if isinstance(frame, pd.DataFrame) and not frame.empty:
                    z.writestr(name, frame.to_csv(index=False).encode("utf-8"))
        buf.seek(0)
        return buf

    summary_pdf = make_summary_page()
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    mode_token = "MLPlantGate" if ml_mode else "BaselineTEA"
    state_token = safe_filename_token(state)
    tech_token = safe_filename_token(tech)
    ...
    export_type = st.radio(
        ...
        (
            ...
            "Download complete researcher dataset",
        ),
        horizontal=True,
    )
    ...
    if export_type.startswith("Scenario PDF"):
        preds48, pred_error = get_export_predictions()
        if pred_error:
        ...
        if state not in preds48.index:
            ...
            st.download_button(
                ...
                buf,
                f"LCOH_{mode_token}_{state_token}_{tech_token}_{year}_{ts}.pdf",
                "application/pdf",
            )
    ...
    # ---------- 2) ZIP with single-page PDFs for all available states ----
        ...
        with st.spinner("Rendering state pages..."):
            with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
                pairs = list(ex.map(_build_one, preds48.index.tolist()))
        ...
        if not pdf_state_pages:
            ...
            st.stop()
        if len(pdf_state_pages) != len(preds48):
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            ...
            for s in sorted(pdf_state_pages):
                z.writestr(f"{safe_filename_token(s)}.pdf", pdf_state_pages[s])
        buf.seek(0)
        st.download_button(
            "Download ZIP with state PDFs",
            buf,
            f"LCOH_{mode_token}_AllStates_{tech_token}_{year}_{ts}.zip",
            "application/zip",
        )

    # ---------- 3) Compact PDF (summary + table) -------------------------
    elif export_type.startswith("Compact PDF"):
        preds48, pred_error = get_export_predictions()
        if pred_error:
            st.warning(pred_error)
            st.stop()
        ...
        table_pdf.cell(0, 10, ascii_safe(f"All States - {lcoh_display_label(calculation_mode)}"), ln=True)
        table_pdf.ln(4)
        ...
        st.download_button(
            ...
            buf,
            f"LCOH_{mode_token}_AllStatesCompact_{tech_token}_{year}_{ts}.pdf",
            "application/pdf",
        )

    else:
        st.caption("Creates a reproducibility ZIP of CSV tables and README assumptions only after you prepare it.")
        researcher_key = f"{mode_token}|{tech_token}|{year}|{','.join(sorted(results.keys())) if isinstance(results, dict) else 'no_models'}"
        if st.button("Prepare complete researcher dataset ZIP", type="primary"):
            with st.spinner("Preparing researcher dataset ZIP..."):
                st.session_state["researcher_dataset_zip"] = build_researcher_dataset_zip().getvalue()
                st.session_state["researcher_dataset_zip_name"] = (
                    f"LCOH_researcher_dataset_{mode_token}_{tech_token}_{year}_{ts}.zip"
                )
                st.session_state["researcher_dataset_zip_key"] = researcher_key

        if st.session_state.get("researcher_dataset_zip_key") == researcher_key:
            st.download_button(
                "Download complete researcher dataset",
                st.session_state["researcher_dataset_zip"],
                st.session_state.get("researcher_dataset_zip_name", f"LCOH_researcher_dataset_{ts}.zip"),
                "application/zip",
            )
