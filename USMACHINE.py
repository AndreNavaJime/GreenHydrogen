...
with tabs[6]:
    ...
    # ---------- Full ZIP bundle (current state only) ---------------------
    ...
    # ---------- NEW: one-page PDF for every state ------------------------
    else:  # "All 48 States (ZIP)"
        st.info("Building one-page PDFs for every state …")
        all_zip = io.BytesIO()
        with zipfile.ZipFile(all_zip, "w") as z:
            for st_name in sorted(df.State.unique()):
                # use the same Tech / Year / slider settings, change only the state
                row_df = df.query(
                ...
                # reuse current CAPEX / efficiency / electricity settings
                cap_val = cap_dynamic
                eff_val = eff_dynamic
                elec_val = row_fix["Electricity_$/kWh"]

                # quick deterministic prediction (same as Baseline TEA path)
                crf = 0.08 * (1 + 0.08) ** 20 / ((1 + 0.08) ** 20 - 1)
                kgH2 = row_fix["CF"] * 8760 / eff_val
                lcoh_det = round(
                    cap_val * crf / kgH2 +
                    cap_val * 0.05 * (cap_val / cap_ref) / kgH2 +
                    elec_val * eff_val +
