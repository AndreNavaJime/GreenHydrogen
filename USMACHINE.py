...
# ─────────────────────────────────────────────────────────────────────────
# TAB 2  –  Feature Importance, SHAP & Trends (with calibration diagnostics)
# ─────────────────────────────────────────────────────────────────────────
with tabs[2]:
    ...
    # Unified light theme for Plotly figures (all text black on white)
    def light_plotly(fig, showgrid=False):
        ...
        # Colorbar (correct nesting for title font)
    ...
    if ml_mode and results and model_name in results:
        ...
        if fi is not None:
            ...
            # Hide the color scale for a cleaner FI bar
    ...
    if ml_mode and results and model_name in results:
        ...
        # Optional: 5-fold out-of-fold predictions
