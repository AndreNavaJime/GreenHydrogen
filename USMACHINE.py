...
with tabs[5]:
    ...
    if national_df.empty:
        ...
        if calculation_mode == "ML Model":
            ...
            for y in years:
                ...
                if not np.isfinite(kgH2):
                    ...
                    # Keep your O&M scaling; switch to 0.05*CAPEX if you prefer a simpler assumption
                    lcoh = (
                        cap_val * crf / kgH2 +
        ...
        # ── Plot (white bg, black text, **no title**) + PNG download ─────
        ...
        st.plotly_chart(fig_nat, use_container_width=True, config={"displaylogo": False})

        # Download exactly this styling as a high‑res PNG (with Kaleido), fallback to HTML
        fn_png = f"National_LCOH_{tech}_{int(national_trend['Year'].min())}-{int(national_trend['Year'].max())}.png"
        try:
            png_bytes = pio.to_image(fig_nat, format="png", scale=3)  # scale>1 = sharper
            st.download_button("⬇️ Download PNG (white background)", png_bytes, file_name=fn_png, mime="image/png")
        except Exception:
            st.info("PNG export requires Kaleido. Install with: `pip install -U kaleido`")
            html_bytes = fig_nat.to_html(full_html=False, include_plotlyjs="cdn").encode("utf-8")
            st.download_button("⬇️ Download HTML (interactive)", html_bytes, file_name=fn_png.replace(".png", ".html"), mime="text/html")
