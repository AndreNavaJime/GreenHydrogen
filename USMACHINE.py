...
# --- Layout Tabs ---
tabs = st.tabs(["Prediction", "Stats", "Visuals", "Map", "Download", "State Comparison", "Optimizer"])
...
with tabs[6]:
    ...
    # Format results into pivot for heatmap
    df_grid = pd.DataFrame(grid, columns=["CAPEX", "Efficiency", "LCOH"])
    df_pivot = df_grid.pivot(index="Efficiency", columns="CAPEX", values="LCOH")

    fig_opt = px.imshow(df_pivot,
                        color_continuous_scale="Viridis",
                        labels={"color":"LCOH ($/kg)"},
                        title="LCOH Heatmap – CAPEX vs Efficiency",
                        template="plotly_dark")
    fig_opt.update_layout(xaxis_title="CAPEX ($/kW)", yaxis_title="Efficiency (kWh/kg)")
