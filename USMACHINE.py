...
def train_models(
    ...
    *,
    ...
    """
    TRAIN MODELS
    Fits selected regressors and reports OOF metrics by default.
    Adds optional 80/20 hold-out metrics. Uses GroupKFold if `groups_col`
    is present (prevents leakage). Returns (results_dict, X_float64).
    """
    import time
    ...
    if (groups is not None) and (len(set(groups)) > 1):
        n_splits = max(2, min(oof_folds, len(set(groups))))
        splitter = GroupKFold(n_splits=n_splits)
        splits = list(splitter.split(X, y, groups=groups))
        cv_note = f"{len(splits)}-fold GroupKFold by {groups_col}"
    else:
    ...
    for name, proto in models.items():
        ...
        for tr_idx, va_idx in splits:
            ...
            y_pred_oof[va_idx] = est.predict(X.iloc[va_idx])
        ...
        # Optional: random hold-out (80/20)
        holdout = {}
        if do_holdout:
            X_tr, X_te, y_tr, y_te = train_test_split(X, y, test_size=0.2, random_state=random_state)
            est_h = clone(proto).fit(X_tr, y_tr)
            y_hat = est_h.predict(X_te)
            holdout = {
        ...
        # Fully-fitted model (on ALL data) for inference in the UI
        inference_model = clone(proto).fit(X, y)

        # SHAP (best-effort; safe fallbacks)
        ...
        try:
            ...
            try:
                ...
                shap_raw  = explainer.shap_values(bg_np)
            except Exception:
                explainer = shap.Explainer(raw_est, bg_np)
                shap_raw  = explainer(bg_np).values
        ...
        results[name] = {
            "model_prototype": proto,              # unfitted template
            "inference_model": inference_model,    # ✅ fitted pipeline for predictions
            "model": inference_model,              # ✅ legacy alias to avoid KeyError
