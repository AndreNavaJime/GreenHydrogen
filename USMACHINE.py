...
with tabs[3]:
    if calculation_mode != "ML Model":
        st.info("Switch to **ML Model** mode to view this map.")
    else:
        # 'pred' must be computed earlier outside tabs
        lcoh = round(pred, 2)

        # Prepare full map
        map_df = gdf.copy()
        map_df["Highlight"] = (map_df["State"] == state)
        map_df["id"] = map_df.index  # Create a stable identifier if needed

        # Plot only the selected state in cyan, rest in gray
        fig_map = px.choropleth(
            map_df,
            geojson=map_df.geometry,
            locations="State",  # Use actual column instead of index
            featureidkey="properties.State",  # Match geometry to State column
            color="Highlight",
            color_discrete_map={True: "cyan", False: "gray"},
            hover_name=None,
            hover_data={"Highlight": False, "State": False},  # ❌ Remove hover data
            template="plotly_dark",
            title=f"{state}: LCOH = ${lcoh:.2f}/kg"
        )

        # Clean styling and remove legend
        ...
        fig_map.update_layout(showlegend=False)  # ✅ Hide legend
