...
def train_models(df: pd.DataFrame):
    ...
    Cleans ANY bracketed/scientific numeric strings BEFORE training/SHAP.
    ...
    def _clean_numeric_df(Xin: pd.DataFrame) -> pd.DataFrame:
        ...
        - Convert to string
        - Remove [brackets], commas, and stray spaces
        - Parse scientific notation (e.g., 4.144483E0)
        - Replace ±inf→NaN, fill with medians→0, enforce float64
        ...
        # drop all-NaN columns before filling
    ...
    X = pd.get_dummies(
        ...
        columns=[c for c in ["Tech"] if c in df_p.columns], drop_first=True
    )
    ...
    # 3) Keep only rows with finite y
    mask = np.isfinite(y)
    ...
    # 5) Tree-based models only (no Linear/Ridge → avoids negative extrapolation)
    models = {
    ...
    for name, model in models.items():
        ...
        # Optional safety clamp (prevents weird negatives in UI)
        ...
        try:
            ...
            # some external estimators may not cross-validate cleanly with numpy/frame mismatch
            cv_r2 = np.nan
