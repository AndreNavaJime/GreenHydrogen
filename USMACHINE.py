...
with tabs[6]:
    ...
    # ---------- Helpers --------------------------------------------------
    lower48 = sorted(gdf.State.unique())           # <-- exactly 48 names
    ...
    def one_page_pdf(state_name, row_fix):
        """Return a bytes object with one-page PDF for the given state."""
        global state
        prev_state, state = state, state_name     # patch header
        pred_val = ml_lcoh(row_fix) if ml_mode else deterministic_lcoh(row_fix)
        pdf_obj  = generate_pdf(row_fix, pred_val, None, full=False)
        state = prev_state
    ...
    # ---------- 3) 48 one-page PDFs → ZIP --------------------------------
        ...
        zip_buf  = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w") as z:
            for i, st_name in enumerate(lower48, 1):
                row_fix = df.query(
