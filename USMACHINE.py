...
# ──────────────────────────────────────────────────────────────────────────
# TAB 6 – Download Center  (48-row fallback, fast, duplicate-proof)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    import io, concurrent.futures, hashlib, zipfile, threading
    from datetime import datetime

    try:
        from PyPDF2 import PdfReader, PdfWriter
    except ModuleNotFoundError:
        st.error("PyPDF2 ≥ 3.0 required → pip install -U PyPDF2")
        st.stop()

    st.subheader("Download Center")

    # canonical order (lower-48)
    lower48 = [
    ...
    # ── build 48-row df_sub (fallback if exact year missing) ──────────────
    rows = []
    for st_name in lower48:
        sel = df.query("State == @st_name and Tech == @tech and Year == @year")
        if sel.empty:
            sel = df.query("State == @st_name and Tech == @tech")
        if sel.empty:
            sel = df.query("State == @st_name")
        if sel.empty:
            st.error(f"No data at all for state: {st_name}")
            st.stop()
        rows.append(sel.iloc[0])

    df_sub = pd.DataFrame(rows).set_index("State")   # exactly 48 rows

    # ── fast vector prediction (cached) ───────────────────────────────────
    @st.cache_resource(show_spinner=False)
    def _predict(df_v: pd.DataFrame) -> pd.Series:
        """
        Bulk-predict 48 states with either ML model or TEA equation,
        using the *current* slider inputs.
        """
        if ml_mode:
            feats = results[model_name]["features"]
            X = pd.DataFrame(0, index=df_v.index, columns=feats)
            X.loc[:, "CF"]                 = df_v["CF"]
            X.loc[:, "CAPEX_$/kW"]         = cap_dynamic
            X.loc[:, "Efficiency_kWh_kg"]  = eff_dynamic
            X.loc[:, "Electricity_$/kWh"]  = df_v["Electricity_$/kWh"]
            X.loc[:, "Water_$kg"]          = df_v["Water_$kg"]
            X.loc[:, "CO2_$kg"]            = df_v["CO2_$kg"]
            X.loc[:, "Transport_$kg"]      = df_v["Transport_$kg"]
            X.loc[:, "Storage_$kg"]        = df_v["Storage_$kg"]
            X.loc[:, "Year_val"]           = year
            X.loc[:, "Year_val_squared"]   = year ** 2
            tc = f"Tech_{tech}"
            if tc in feats:
                X[tc] = 1
            X = sanitize(X, feats)
        ...
        return pd.Series(preds, index=df_v.index, dtype="float64")

    preds = _predict(df_sub)

    # ── helper to build one-pager (thread-safe) ───────────────────────────
    _lock = threading.Lock()   # guards global “state”

    def build_one(st_name: str) -> tuple[str, bytes]:
        row  = df_sub.loc[st_name]
        pred = float(preds[st_name])

        with _lock:
            prev = globals().get("state")
            globals()["state"] = st_name
            raw = (
                generate_pdf(row, pred, None, full=False)
                .output(dest="S")
                .encode("latin1")
            )
            if prev is None:
        ...
        if len(PdfReader(io.BytesIO(raw)).pages) != 1:
            raise ValueError(f"{st_name}: generate_pdf produced ≠1 page")
        return st_name, raw

    # ── parallel render (8 threads) ───────────────────────────────────────
    with st.spinner("Rendering 48 one-page PDFs…"):
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as ex:
            pdf_bytes_by_state = dict(ex.map(build_one, lower48))

    # duplicate guard – should always pass now
    if len({hashlib.md5(b).digest() for b in pdf_bytes_by_state.values()}) != 48:
    ...
    choice = st.radio(
        "Choose export type",
        ("Scenario PDF", "ZIP (48 PDFs)", "Single PDF (48 pages)"),
        horizontal=True
    )

    if choice == "Scenario PDF":
        scenario_pdf = generate_pdf(
            df_sub.loc[state], float(preds[state]), None, full=False
    ...
    else:  # Single PDF (48 pages)
        out = PdfWriter()
        for s in lower48:
            out.add_page(PdfReader(io.BytesIO(pdf_bytes_by_state[s])).pages[0])
        buf = io.BytesIO(); out.write(buf); buf.seek(0)
        st.download_button("Download 48-page PDF", buf,
