...
state_rate = state_rate_cents / 100
...
with tabs[4]:
    ...
    elec_factors = [0.9, 0.95, 1.0, 1.05, 1.1]  # NEW: add variation on electricity
    ...
    for st_name in sorted(df.State.unique()):
        ...
        for c, e, ep in zip(cap_values, eff_values, elec_factors):
            row = {
                ...
                "Electricity_$/kWh": round(elec_base * ep, 4),
    ...
    summary_stats = (
        ...
        .applymap(lambda x: float(f"{x:.2f}"))  # Ensure all values are floats
    )
