...
def train_models(
    ...
    *,
    ...
    random_state: int = 42,
    show_title: bool = False,          # ← stays quiet in the app
    verbose_debug: bool = False,
):
    """
    TRAIN MODELS (code anchor only; no app title)
    - Renames SEC_kWh_kg→Efficiency_kWh_kg, LCOH_plantgate_$kg→LCOH_$kg
    - Drops leakage columns (C_* costs, LCOH_* bands, extra lcoh_* fields)
    - Uses log1p/expm1 target transform to keep ŷ ≥ 0 (with final clip)
    ...
    # Rename to the canonical names your UI expects
    rename_map = {}
    ...
    # Drop leakage columns that contain the answer (or parts of it)
    leak_cols = []
    ...
    # Create canonical drivers if missing (map common synonyms)
    if "Efficiency_kWh_kg" not in X.columns and "Efficiency_kWh_kg" in df_p.columns:
        X["Efficiency_kWh_kg"] = pd.to_numeric(df_p["Efficiency_kWh_kg"], errors="coerce")
    if "CAPEX_$/kW" not in X.columns and "CAPEX_$/kW" in df_p.columns:
        X["CAPEX_$/kW"] = pd.to_numeric(df_p["CAPEX_$/kW"], errors="coerce")
    if "Electricity_$/kWh" not in X.columns and "Electricity_$/kWh" in df_p.columns:
        X["Electricity_$/kWh"] = pd.to_numeric(df_p["Electricity_$/kWh"], errors="coerce")
    ...
    # -------- 3) Model zoo (log1p target transform) ----------------------
    def _ttr(reg):
    ...
    for name, proto in models.items():
        ...
        for tr_idx, va_idx in splits:
            ...
            y_pred_oof[va_idx] = np.clip(y_hat, 0.0, None)  # final safety clip
        ...
        try:
            ...
            # unwrap inner regressor from TTR
        ...
        results[name] = {
            ...
            "model": inference_model,                 # legacy alias for your UI
