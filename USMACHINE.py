...
# (thread-safe, duplicate-free, summary page first, ASCII-only text)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    ...
    # ---------- small helper: strip any non-Latin-1 character -------------
    def ascii_safe(txt: str) -> str:
        # Replace common unicode punctuation, then drop the rest
        txt = (txt.replace("–", "-").replace("—", "-")
                   .replace("’", "'").replace("“", '"').replace("”", '"'))
        return unicodedata.normalize("NFKD", txt).encode("latin1", "ignore").decode("latin1")

    # Canonical lower-48 list (unchanged)
    lower48 = [
    ...
    # ── 1) Build df_sub: one row per state ────────────────────────────────
    rows = []
    ...
    # ── 6) Branch per export option (identical logic, ASCII-safe titles) ─
    if export_type == "Scenario PDF":
        writer = PdfWriter()
        writer.append(PdfReader(io.BytesIO(summary_pdf)))
        writer.append(PdfReader(io.BytesIO(make_state_page(state, float(preds48[state])))))
        ...
        table_pdf.ln(4)
        table_pdf.set_font("Arial", "B", 12); table_pdf.cell(50, 8, "State", 1); table_pdf.cell(40, 8, "LCOH ($/kg)", 1); table_pdf.ln()
