...
# ──────────────────────────────────────────────────────────────────────────
#  TAB 6  –  Download Center  (clean 48-state export, no global keyword)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    ...
    # ── Definitive list of the 48 contiguous states ──────────────────────
    lower48 = [
    ...
    # ── One unique row per state (no duplicates) ─────────────────────────
    ...
    # ── Build exactly 48 one-page PDFs (no caching) ──────────────────────
    pdf_bytes_by_state = {}
    for st_name in lower48:
        row, pred_val = df_sub.loc[st_name], float(preds_by_state[st_name])

        # Temporarily patch the header variable used by generate_pdf()
        ...
        pdf_bytes_by_state[st_name] = pdf_bytes

    # Defensive check: identical byte hashes → duplicate page
    if len({hash(b) for b in pdf_bytes_by_state.values()}) != 48:
        st.warning("⚠️ Detected identical pages – export aborted.")
        st.stop()
