...
def load_electricity_prices():
    file_path = r"C:\Users\Navar\OneDrive\Documentos\USA\CSV TIF\ElectricityUSA.xlsx"
...
def train_models(df):
    if len(df) < 10:
        return {}
    X = pd.get_dummies(df.drop(columns=["State", "LCOH_$kg", "Electricity_$/kWh_fixed"]), columns=["Tech"], drop_first=True)
    y = df["LCOH_$kg"]
