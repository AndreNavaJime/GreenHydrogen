...
st.markdown("""
<style>
.stApp { background-color: #111; color: #F5F5F5; }
...
model_name = st.sidebar.selectbox("Choose Model", list(results.keys()), index=0)
...
with tabs[2]:
    ...
    if hasattr(best, "feature_importances_"):
        fi = pd.Series(best.feature_importances_, index=X_train.columns).sort_values()
        fig_bar = px.bar(
...
with tabs[4]:
    y_test_fixed = results[model_name]["y_test"]
    df_out = pd.DataFrame({
        "True": y_test_fixed,
        "Predicted": results[model_name]["preds"],
        "Error": y_test_fixed - results[model_name]["preds"]
    })
