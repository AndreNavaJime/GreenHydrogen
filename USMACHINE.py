...
with tabs[2]:
    ...
    # local “white PNG” helper — any Kaleido error is caught & bubbled up
    import copy, plotly.io as pio
    def try_png(fig, scale=2):
        """
        Return PNG bytes or raise RuntimeError if Kaleido / Chrome unavailable.
        """
        f = copy.deepcopy(fig)
        f.update_layout(template="plotly_white",
                        ...
                        plot_bgcolor="white",
                        title=None)
        return pio.to_image(f, format="png", scale=scale)   # may throw
    ...
    if ml_mode and "best" in globals():
        ...
        if fi is not None:
            ...
            # — download (if Kaleido OK) —
            try:
                bar_png = try_png(fig_bar)
                st.download_button(
                    "Download PNG (white, no title)",
                    bar_png,
    ...
    # — download heat-map (if Kaleido OK) —
    try:
        hm_png = try_png(fig_hm)
        st.download_button(
            "Download heat-map PNG (white, no title)",
            hm_png,
