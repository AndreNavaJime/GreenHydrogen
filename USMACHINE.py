...
with tabs[6]:
    ...
    # ---------- Static list of contiguous states ------------------------
    lower48 = [
    ...
    # ---------- One unique row per state --------------------------------
    df_sub = (
        df.query("Tech == @tech and Year == @year")
          .sort_values("State")           # deterministic
          .groupby("State", as_index=False)
          .first()                        # exactly one row/state
    )
    ...
    # ---------- LCOH calculators ----------------------------------------
    def deterministic_lcoh(row):
        crf  = 0.08 * (1 + 0.08)**20 / ((1 + 0.08)**20 - 1)
        kgH2 = row["CF"] * 8760 / eff_dynamic
        return round(
            cap_dynamic * crf / kgH2
            + cap_dynamic * 0.05 * (cap_dynamic / cap_ref) / kgH2
            + row["Electricity_$/kWh"] * eff_dynamic
            + row["Water_$kg"] + row["CO2_$kg"]
            + row["Transport_$kg"] + row["Storage_$kg"],
            4
        )
    ...
    def ml_lcoh(row):
        ...
        X_row.update({
            ...
            "CAPEX_$/kW": cap_dynamic,
            "Efficiency_kWh_kg": eff_dynamic,
            "Electricity_$/kWh": row["Electricity_$/kWh"],
            "Water_$kg": row["Water_$kg"],
            "CO2_$kg": row["CO2_$kg"],
            "Transport_$kg": row["Transport_$kg"],
            "Storage_$kg": row["Storage_$kg"],
            "Year_val": year,
            "Year_val_squared": year**2,
        })
        for f in X_row:
    ...
    # ---------- PDF helper ----------------------------------------------
    def one_page_pdf(state_name, row):
        global state
        prev_state, state = state, state_name
        pred_val = ml_lcoh(row) if ml_mode else deterministic_lcoh(row)
        pdf_obj  = generate_pdf(row, pred_val, fig_path, full=False)
        state = prev_state
    ...
    # ---------- 4) 48-page single PDF -----------------------------------
        ...
        for i, st_name in enumerate(lower48, 1):
            progress.progress(i / 48)
            page_pdf = one_page_pdf(st_name, rows_by_state[st_name])
