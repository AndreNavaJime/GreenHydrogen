...
def train_models(
    ...
    *,
    ...
    # NEW:
    speed: str = "fast",           # "fast" | "standard" | "thorough"
    shap_mode: str = "off",        # "off" | "selected" | "all"
):
    """
    TRAIN MODELS (code anchor only; no app title)
    - Harmonizes column names, drops leakage columns.
    - Target transform log1p/expm1 to keep ŷ ≥ 0 (clip safety).
    - Speed profiles for ALL models; SHAP optional.
    ...
    # ensure core drivers exist if present in df_p
    ...
    for m_i, (name, proto) in enumerate(models.items(), start=1):
        ...
        # SHAP (optional; small background for speed)
        ...
        if shap_mode != "off":
            try:
                ...
                if shap_mode == "all" or (shap_mode == "selected" and name in ("XGBoost","LightGBM","RandomForest","GBRegressor")):
                    try:
