...
with tabs[2]:
    ...
    # ───────────── ML-only visuals (no messages in baseline mode) ─────────────
    if ml_mode and results and model_name in results:
        ...
        if fi is not None:
            ...
            fig_bar = px.bar(
                ...
                labels={"x": "Importance"},  # no y label to avoid overlap
                template="none"
            )
            # Hide color scale; remove y-title; give room for long labels
            fig_bar.update_layout(
                coloraxis_showscale=False,
                yaxis_title=None,
                margin=dict(l=180, r=10, t=10, b=40)
            )
        ...
        with st.expander("Residual QQ plot (optional)"):
            try:
                ...
                pass  # stay quiet if SciPy isn't available
