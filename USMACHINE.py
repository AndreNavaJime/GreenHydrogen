...
def train_models(df: pd.DataFrame):
    """
    Fit several regressors quickly and return (results, X_train).
    - Uses float32 features
    - No SHAP here (computed lazily later)
    - 3-fold CV only
    - XGBoost uses hist + early stopping
    """
    import time
    import numpy as np
    ...
    from sklearn.ensemble import RandomForestRegressor, GradientBoostingRegressor
    from xgboost import XGBRegressor
    # (LightGBM / CatBoost omitted here for speed; re-enable later if needed)
    ...
    # 2) Clean + cast to float32 (big speedup) -------------------------------
    X = (
    ...
    y = pd.to_numeric(y, errors="coerce").replace([np.inf, -np.inf], np.nan).fillna(y.median()).astype("float32")

    # 3) Filter non-finite y --------------------------------------------------
    mask = np.isfinite(y)
    X, y = X[mask], y[mask]
    ...
    # 5) Models (lean set) ----------------------------------------------------
    models = {
        "Linear": LinearRegression(),  # basically free
        ...
        "XGBoost": XGBRegressor(
            n_estimators=800,                 # paired with early stopping
            learning_rate=0.05,
            max_depth=6,
            subsample=0.9,
    ...
    # 6) Early stopping for XGB (uses a slice of train as validation) --------
    #    We keep X_te strictly for final metrics.
    from sklearn.model_selection import train_test_split as _tts
    X_tr2, X_val, y_tr2, y_val = _tts(X_tr, y_tr, test_size=0.2, random_state=42)
    ...
    for name, model in models.items():
        ...
        # quick 3-fold CV (lighter than 5-fold)
        ...
        results[name] = {
            "model": model,
            "r2": float(np.corrcoef(y_te, preds)[0,1]**2) if np.std(preds) > 0 else 0.0,
            "rmse": float(np.sqrt(((y_te - preds) ** 2).mean())),
            "mae": float(np.abs(y_te - preds).mean()),
            "cv_r2": float(cv_r2),
            "preds": preds.tolist(),
            "y_true": y_te.tolist(),
            "features": X.columns.tolist(),
            "explainer": None,      # set later (lazy SHAP)
            "shap_values": None,    # set later (lazy SHAP)
        }
...
# ── Train models  ──────────────────────────────────────────
train_col1, train_col2 = st.sidebar.columns([1,1])
do_train = train_col1.button("🔁 Train/Refresh models", use_container_width=True)
show_stats = train_col2.checkbox("Auto-train on load", value=False)

if do_train or ("ml_out" not in st.session_state and show_stats):
    out = train_models(df)
    st.session_state.ml_out = out

out = st.session_state.get("ml_out", None)

ml_available = isinstance(out, tuple) and len(out) == 2 and bool(out[0])
if ml_available:
    results, X_train = out
else:
    results, X_train = {}, None  # Baseline TEA still works
...
@st.cache_resource(show_spinner=False)
def compute_shap_cached(model_name: str, model, X_train, max_samples: int = 300):
    """
    Fast, cached SHAP for tree/linear models on a small sample.
    Returns (values, X_used, expected_value, pretty_feature_names).
    """
    import numpy as np, shap

    # sample rows
    if hasattr(X_train, "iloc"):
        Xs = X_train.sample(min(max_samples, len(X_train)), random_state=42)
    else:
        idx = np.random.default_rng(42).choice(X_train.shape[0], size=min(max_samples, X_train.shape[0]), replace=False)
        Xs = X_train[idx]

    # pick explainer type
    if model_name.lower() in {"linear", "ridge"}:
    ...
    exp_val = explainer.expected_value if hasattr(explainer, "expected_value") else 0.0
    return vals, Xs, exp_val, pretty
