...
param_source = st.sidebar.radio(
    ...
    horizontal=True,
    help=f"DOE targets pulls CAPEX & efficiency projections from the DOE: {tech_info.get(tech)}"
)
apply_doe = (param_source == "DOE targets")

# ► defaults (DOE values or last‐tweaked CSV values)
cap_def  = adjust_capex_by_year(tech, year) if apply_doe else row_base["CAPEX_$/kW"]
#— FIXED: use the exact column name from df!
