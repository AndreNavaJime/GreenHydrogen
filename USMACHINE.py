...
with tabs[2]:
    ...
    with tabs[3]:st.subheader("Map: State-Level LCOH Prediction")

    # Step 1: Build dynamic rows for all states with proper year and tech encoding
    dynamic_rows = []
    for st_name in sorted(df.State.unique()):
        try:
            subset = df.query("State == @st_name and Year == @year and Tech == @tech")
            if subset.empty:
                continue
        ...
        # Add all year columns expected by the model; only selected year is set to 1
    ...
    # Step 2: Feature alignment helper
