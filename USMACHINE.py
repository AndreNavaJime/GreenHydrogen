...
eff_input = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max), round((eff_min + eff_max) / 2, 1))
elec = st.sidebar.slider(
    "Electricity price ($/kWh)",
    0.01, 0.30,
    float(round(df.loc[df.State == state, "Electricity_$/kWh_fixed"].iloc[0], 4))
)
...
row = {
    ...
    "CAPEX_$/kW": cap,  # 👈 use slider
    "Efficiency_kWh_kg": eff_input,  # 👈 use slider
    ...
    # ✅ Restore this — these drive the year extrapolation logic
    "Year_val": year,
...
# ---  Scenario Prediction Setup (Tabs 0 and 1) ---
try:
