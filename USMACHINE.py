...
with tabs[6]:
    ...
    # ───────── Optional residual plot (ML mode) ──────────────────────────
    fig_path = None
    if ml_mode:
    ...
    # ---------- helper: fast deterministic LCOH --------------------------
    def quick_lcoh(row_fix):
        """Deterministic cost with *current* sidebar sliders."""
        crf  = 0.08 * (1 + 0.08) ** 20 / ((1 + 0.08) ** 20 - 1)
        ...
        return round(
            ...
            row_fix["Water_$kg"] + row_fix["CO2_$kg"] +
            row_fix["Transport_$kg"] + row_fix["Storage_$kg"], 4
        )

    # ---------- helper: one ML prediction --------------------------------
    def ml_predict(row_fix):
        """One-row ML prediction using current model."""
        X_one = {k: 0 for k in results[model_name]["features"]}
        # numeric features
        X_one.update({
            "CF": row_fix["CF"],
            "CAPEX_$/kW": cap_dynamic,
        ...
        for f in X_one:
            if f.startswith("Tech_"):
                X_one[f] = 1 if f == f"Tech_{tech}" else 0
        df_one = pd.DataFrame([X_one])[results[model_name]["features"]]
        df_one = sanitize(df_one, results[model_name]["features"])
        return float(results[model_name]["model"].predict(df_one)[0])

    # ---------- 1) current scenario only ---------------------------------
    if download_mode == "Scenario Report Only":
        ...
        buf.seek(0)
        st.download_button("Download Scenario PDF", buf,
                           file_name=f"Scenario_Report_{timestamp}.pdf",
                           mime="application/pdf", key="dl_scen")

    # ---------- 2) current state – full bundle ---------------------------
    elif download_mode == "Full ZIP + Report":
        pdf_full = generate_pdf(row, pred, fig_path, full=True)
        zip_buf = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w") as z:
            z.writestr(f"LCOH_Full_Report_{timestamp}.pdf",
        ...
        st.download_button("Download Full ZIP", zip_buf,
                           ...
                           mime="application/zip", key="dl_full")

    # ---------- 3) 48 one-page PDFs → ZIP --------------------------------
    elif download_mode == "All 48 States (ZIP)":
        st.info("Generating one-page PDFs for every state …")
        all_zip = io.BytesIO()
        with zipfile.ZipFile(all_zip, "w") as z:
            prev_state = state   # preserve sidebar selection
            for st_name in df.State.unique():
                row_fix = df.query(
                    "State == @st_name and Year == @year and Tech == @tech"
                ).iloc[0]

                state = st_name                          # patch global
                pred_state = (ml_predict(row_fix) if ml_mode
                              else quick_lcoh(row_fix))
                pdf_state = generate_pdf(row_fix, pred_state, None, full=False)
                state = prev_state                       # restore

                z.writestr(f"{st_name}_{tech}_{year}.pdf",
                           pdf_state.output(dest="S").encode("latin1"))
        all_zip.seek(0)
        st.download_button("Download 48-state ZIP", all_zip,
                           file_name=f"AllStates_{tech}_{year}_{timestamp}.zip",
                           mime="application/zip", key="dl_zip")

    # ---------- 4) 48 states in one multi-page PDF ------------------------
        ...
        st.info("Compiling a single PDF for all states …")
        writer = PdfWriter()
        prev_state = state
        ...
        single_buf.seek(0)
        st.download_button("Download 48-state PDF", single_buf,
