...
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    import io, concurrent.futures, hashlib, zipfile
    ...
    # ── build 48-row df_sub (fallback to any row for missing year) ───────
    rows = []
    for st_name in lower48:
        # exact Tech + Year first
        sel = df.query("State == @st_name and Tech == @tech and Year == @year")
        if sel.empty:
            # fallback: same Tech, any Year
            sel = df.query("State == @st_name and Tech == @tech")
        if sel.empty:
            # final fallback: any row for the state
    ...
    df_sub = pd.DataFrame(rows).set_index("State")  # exactly 48 rows in lower48 order
    ...
    def build_one(st_name: str) -> tuple[str, bytes]:
        ...
        if len(pdf.pages) != 1:
            raise ValueError(f"{st_name}: generate_pdf returned {len(pdf.pages)} pages")
        return st_name, raw
