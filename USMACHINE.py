...
def train_models(
    ...
    *,
    ...
    """
    TRAIN MODELS (quiet in UI)
    - Harmonizes names: SEC_kWh_kg→Efficiency_kWh_kg, LCOH_plantgate_$kg→LCOH_$kg
    - Drops leakage columns (C_* terms, extra lcoh_* bands)
    - Ensures core drivers exist (CAPEX_$/kW, Efficiency_kWh_kg) via DOE backfill
    - Uses log1p/expm1 target transform (ŷ ≥ 0) + final nonneg clip for metrics
    ...
    # Canonical names
    rename_map = {}
    if "SEC_kWh_kg" in df_p.columns:
        rename_map["SEC_kWh_kg"] = "Efficiency_kWh_kg"
    if "LCOH_plantgate_$kg" in df_p.columns:
        rename_map["LCOH_plantgate_$kg"] = "LCOH_$kg"
    df_p = df_p.rename(columns=rename_map)

    # Remove leakage (anything that directly composes LCOH or alternate LCOH cols)
    leak_cols = []
    for c in df_p.columns:
        cl = c.lower()
        if c.startswith(("C_capex_", "C_opex_", "C_repl_", "C_elec_")):
            leak_cols.append(c)
        elif cl.startswith("lcoh_") and c != "LCOH_$kg":
            leak_cols.append(c)
    if leak_cols:
        df_p = df_p.drop(columns=leak_cols, errors="ignore")

    # ---------- 2) Backfill core drivers if missing ----------
    # Helpers that use your DOE curves (already defined elsewhere in your app)
    def _cap_doe(row):
        try:
            return float(adjust_capex_by_year(str(row.get("Tech")), int(row.get("Year"))))
        except Exception:
            return np.nan

    def _eff_doe(row):
        try:
            return float(get_efficiency_by_year(str(row.get("Tech")), int(row.get("Year"))))
        except Exception:
            return np.nan

    if "Efficiency_kWh_kg" not in df_p.columns:
        df_p["Efficiency_kWh_kg"] = df_p.apply(_eff_doe, axis=1)
    if "CAPEX_$/kW" not in df_p.columns:
        df_p["CAPEX_$/kW"] = df_p.apply(_cap_doe, axis=1)

    # Coerce to numeric
    for c in ("Efficiency_kWh_kg", "CAPEX_$/kW", "Electricity_$/kWh"):
    ...
    # Ensure the core drivers are present in X (even if one-hot/dtype ops dropped them)
    for core in ("CAPEX_$/kW", "Efficiency_kWh_kg"):
        if core not in X.columns and core in df_p.columns:
            X[core] = pd.to_numeric(df_p[core], errors="coerce")

    # Clean numerics
    S = X.astype(str).replace(r"\[|\]", "", regex=True).apply(lambda s: s.str.strip())
    X = (S.apply(pd.to_numeric, errors="coerce")
    ...
    # Warn if we still failed to include the core drivers (so Tab-4 can know to fall back)
    missing_core = [c for c in ("CAPEX_$/kW","Efficiency_kWh_kg") if c not in X.columns]
    if missing_core:
        st.sidebar.warning(f"Core drivers missing from model features: {missing_core}. "
                           "State sweep will use TEA fallback.")

    # ---------- 4) Model zoo (log target) ----------
    def _ttr(reg):
    ...
    for name, proto in models.items():
        ...
        for tr_idx, va_idx in splits:
            ...
            # ŷ already on original scale via TTR; clip ≥ 0 for reported metrics
        ...
        results[name] = {
            ...
            **holdout,
            "explainer": explainer, "shap_values": shap_vals,
        }
