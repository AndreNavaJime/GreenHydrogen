...
def load_data():
    ...
    for tech, csv_name in [
        ...
        d = pd.read_csv(fp)
        tgt = [c for c in d.columns if "LCOH" in c.upper()][0]   # find LCOH col
    ...
    # ---------- 2) FEATURE tables ----------
    elec = load_electricity_prices()              # prices

    ghi = read_quoted_csv(
        "GHIUSA.csv",
        ["State", "GHI_min", "GHI_max",
         "GHI_mean", "GHI_median", "GHI_std"]
    )

    wind = read_quoted_csv(
        "WINDUSA.csv",
        ["State", "Wind_min", "Wind_max",
         "Wind_mean", "Wind_median", "Wind_std"]
    )

    # --- GRID DISTANCE (make sure it has a proper 'State' column) ---
    grid = (
        pd.read_csv("STATEGRIDDISTANCE.csv")       # raw read
          .rename(columns=lambda c: c.strip())     # trim whitespace
    )
    if 'State' not in grid.columns:               # auto-rename if needed
        for c in grid.columns:
            if c.lower().startswith('state'):
                grid = grid.rename(columns={c: 'State'})
                break
    # grid now has ['State', 'GridDist_km']

    water = (
        pd.read_csv("USWATER.csv")
          .rename(columns={"NAME": "State",
                           "runoff_km3": "Runoff_km3",
                           "runoff_mm":  "Runoff_mm"})
          [["State", "Runoff_km3", "Runoff_mm"]]
    )

    land = (
        pd.read_csv("USLAND.csv")
          .rename(columns={"NAME": "State",
                           "solar": "Solar_idx",
                           "wind":  "LandWind_idx",
                           "suitability": "Suitability_idx"})
          [["State", "Solar_idx", "LandWind_idx", "Suitability_idx"]]
    )
    ...
    co2 = (
        pd.read_csv("USACO2.csv")
          [["State", "2023 (million metric tons CO₂)"]]
          .rename(columns={"2023 (million metric tons CO₂)": "CO2_2023_Mt"})
    )

    biomass = (
        pd.read_csv("USABIOMASS.csv")
          .rename(columns={"NAME": "State", "AGB_Mg_ha": "AGB_Mg_ha"})
    )

    # ---------- 3) MERGE all features (left-joins so we never drop a state) ----------
    features_df = (
        elec
...
out = train_models(df)
if isinstance(out, tuple):          # normal case
    results, X_train = out
else:                               # train_models returned {}
...
# fill any extra feature columns the model expects
for extra in (set(results[model_name]["features"]) - set(row)):
...
with tabs[4]:
    ...
    features = results[model_name]["features"] if calculation_mode == "ML Model" else []
    ...
    for st_name in sorted(df.State.unique()):
        ...
        for c, e in zip(cap_values, eff_values):
            ...
            if calculation_mode == "ML Model":
                row.update({"Year_val": year, "Year_val_squared": year**2})
                for col in features:
                ...
                row["LCOH ($/kg)"] = float(results[model_name]["model"].predict(df_row)[0])

            else:  # True Equation Mode
                crf = 0.08 * (1 + 0.08) ** 20 / ((1 + 0.08) ** 20 - 1)
                kgH2 = cf_val * 8760 / row["Efficiency_kWh_kg"]
                row["LCOH ($/kg)"] = round(
                    row["CAPEX_$/kW"] * crf / kgH2 +
                    row["CAPEX_$/kW"] * 0.05 * (row["CAPEX_$/kW"] / cap_ref) / kgH2 +
    ...
    if not variations:
        st.warning("No data available for selected state/tech/year in True Equation mode.")
    else:
