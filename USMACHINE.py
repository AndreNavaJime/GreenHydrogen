...
with tabs[4]:
    ...
    if np.isnan(cap_dynamic) or np.isnan(eff_dynamic):
        st.error("❌ CAPEX or Efficiency inputs are invalid.")
        st.stop()

    # Generate variation ranges
    ...
    st.caption("🔍 Testing variations:")
    st.write("CAPEX values:", cap_values)
    st.write("Efficiency values:", eff_values)

    features = results[model_name]["features"] if ml_mode else []

    if ml_mode and not {"CAPEX_$/kW", "Efficiency_kWh_kg"}.intersection(set(features)):
        st.warning("⚠️ Model does not include CAPEX or Efficiency as features — variations may not affect prediction.")
    ...
    for st_name in sorted(df.State.unique()):
        ...
        for c, e in product(cap_values, eff_values):  # 25 combos
            row_sim = {
                "State": st_name,
                "CF": cf_val,
                "CAPEX_$/kW": float(c),
            ...
            if ml_mode:
                ...
                pred = results[model_name]["model"].predict(df_row)[0]
                row_sim["LCOH ($/kg)"] = float(pred)
            else:
                crf = 0.08 * (1 + 0.08) ** 20 / ((1 + 0.08) ** 20 - 1)
    ...
    if not variations:
        st.warning("⚠️ No data available for the selected state/tech/year.")
    else:
        df_var = pd.DataFrame(variations)

        # Optional: show sample predictions
        st.write("🧪 Sample simulated rows (first 3):")
        st.dataframe(df_var.head(3))

        # Summary stats
        summary_stats = (
            df_var.groupby("State")["LCOH ($/kg)"]
                 .agg(mean="mean", min="min", max="max", std=lambda s: s.std(ddof=1))
                 .round(2)
        )
