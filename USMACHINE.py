...
cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max), value=int(cap_default),
                        step=1, key="cap_val", disabled=auto_mode)
eff_input = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max),
                              value=float(eff_default), step=0.1, key="eff_val", disabled=auto_mode)
elec = st.sidebar.slider("Electricity price ($/kWh)", 0.01, 0.30,
                         value=elec_default, key="elec_val")
