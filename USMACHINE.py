...
# (2)  Parameter source: DOE targets vs. Manual override
tech_info = {
...
param_source = st.sidebar.radio(
    "Parameter source",
    ["DOE targets", "Manual override"],
    horizontal=True,
    help=f"DOE targets pulls CAPEX & efficiency projections from the DOE: {tech_info[tech]}"
)
...
# ► ensure we have session keys (first run only)
...
# ► if in DOE mode, overwrite session_state so sliders reset
if apply_doe:
    st.session_state.cap        = cap_official
    st.session_state.eta        = eff_official
    st.session_state.elec_price = elec_official

# ► sliders read/write from session_state
cap_dynamic = st.sidebar.slider(
    "CAPEX ($/kW)",
    int(cap_min), int(cap_max),
    int(st.session_state.cap),
    step=1,
    key="cap"           # binds to st.session_state.cap
)
eff_dynamic = st.sidebar.slider(
    "Efficiency (kWh/kg)",
    float(eff_min), float(eff_max),
    float(st.session_state.eta),
    step=0.1,
    key="eta"           # binds to st.session_state.eta
)
elec = st.sidebar.slider(
    "Electricity price ($/kWh)",
    0.01, 0.30,
    float(round(st.session_state.elec_price,4)),
    step=0.01,
    key="elec_price"    # binds to st.session_state.elec_price
)
...
# ──────────────────────────────────────────────────────────────────────────
# TAB 6 – Download Center  (ASCII-only text; no SHAP images)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    ...
    # ── 1) Build a 48-row subset matching sidebar inputs ───────────────────
    rows = []
    for st_name in lower48:
        sel = df.query("State == @st_name and Tech == @tech and Year == @year")
        if sel.empty: sel = df.query("State == @st_name and Tech == @tech")
        if sel.empty: sel = df.query("State == @st_name")
        if sel.empty:
            st.error(f"No data at all for state: {st_name}"); st.stop()
        rows.append(sel.iloc[0])
    df_sub = pd.DataFrame(rows).set_index("State")

    # ── 2) Predict vector of 48 LCOH values (cached) ───────────────────────
    ...
    # ── 3) PDF helpers (summary + per-state pages) ─────────────────────────
    def make_summary_page() -> bytes:
        pdf = FPDF(); pdf.add_page()
        pdf.set_font("Arial", "B", 16)
        pdf.cell(0, 10, ascii_safe("LCOH Scenario - Inputs"), ln=True)
        pdf.set_font("Arial", "", 12)
        pdf.multi_cell(
            0, 10,
            ascii_safe(
                ...
                f"Technology: {tech}\n"
                f"Year: {year}\n"
                f"CAPEX slider: ${cap_dynamic:,.0f}/kW\n"
                f"Efficiency slider: {eff_dynamic:.2f} kWh/kg\n"
                f"Electricity price slider: ${elec:.4f}/kWh\n"
            )
        )
    ...
    # ── 4) Render 48 state pages in parallel ───────────────────────────────
    ...
    if len({hashlib.md5(b).digest() for b in pdf_state_pages.values()}) != 48:
        st.error("Duplicate PDF pages detected – export aborted."); st.stop()
    ...
    export_type = st.radio(
        ...
        ("Scenario PDF",
         "ZIP (48 PDFs)",
         "Single PDF (Summary + 48 pages)",
         "Compact PDF (Summary + table)"),
        horizontal=True
    )

    # ── 6) Branch per export option (no images anywhere) ───────────────────
    if export_type == "Scenario PDF":
        ...
        with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
            ...
            for s, b in pdf_state_pages.items(): z.writestr(f"{s}.pdf", b)
        buf.seek(0)
        st.download_button(
    ...
    elif export_type == "Single PDF (Summary + 48 pages)":
        writer = PdfWriter()
        writer.append(PdfReader(io.BytesIO(summary_pdf)))
        for s in lower48:
            writer.append(PdfReader(io.BytesIO(pdf_state_pages[s])))
        buf = io.BytesIO(); writer.write(buf); buf.seek(0)
        st.download_button(
            "Download full 50-page PDF",
            buf,
            f"AllStatesFull_{tech}_{year}_{ts}.pdf",
            "application/pdf"
        )
        ...
        table_pdf.cell(0, 10, ascii_safe("All States - Predicted LCOH ($/kg)"), ln=True)
        table_pdf.ln(4)
        table_pdf.set_font("Arial", "B", 12)
        table_pdf.cell(50, 8, "State", 1); table_pdf.cell(40, 8, "LCOH ($/kg)", 1); table_pdf.ln()
        ...
        writer = PdfWriter()
        writer.append(PdfReader(io.BytesIO(summary_pdf)))
        writer.append(PdfReader(io.BytesIO(table_pdf.output(dest="S").encode("latin1"))))
        buf = io.BytesIO(); writer.write(buf); buf.seek(0)
        st.download_button(
