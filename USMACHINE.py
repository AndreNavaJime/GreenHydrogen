...
with tabs[0]:
    ...
    # ── Scenario-only PDF (exactly 1 page) ───────────────────────────────
    with st.expander("Download this scenario only", expanded=False):
        pdf_one = generate_pdf(row, pred, fig_path=None, full=False)
        ...
        st.download_button(
            ...
            data=buf,
            file_name=f"Scenario_{state}_{tech}_{year}.pdf",
            mime="application/pdf",
            key="scenario_pdf_btn"    # unique key avoids clashes
        )
