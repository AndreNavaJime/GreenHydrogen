...
# ──────────────────────────────────────────────────────────────────────────
# TAB 6 – Download Center  (fast, duplicate-proof, gap-proof)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    import io, concurrent.futures, hashlib

    # --- dependency guard (import only once) -----------------------------
    try:
        from PyPDF2 import PdfReader, PdfWriter
    except ModuleNotFoundError:
        st.error("PyPDF2 ≥ 3.0 is required.\nRun  pip install -U PyPDF2  and restart.")
        st.stop()
    ...
    # --- build df_sub : 1 row / state, right order ----------------------
    tmp        = df.query("Tech == @tech and Year == @year")
    dupes      = tmp["State"][tmp["State"].duplicated()].unique().tolist()
    if dupes:
        st.error("❌ Duplicate rows in source data: " + ", ".join(dupes))
        st.stop()

    df_sub = (tmp.drop_duplicates("State", keep="first")
                  .set_index("State")
                  .reindex(lower48))               # force order

    if df_sub.isnull().any(axis=None):
        missing = df_sub[df_sub.isnull().any(axis=1)].index.tolist()
        st.error("❌ Missing data rows for: " + ", ".join(missing))
        st.stop()

    # --- fast vector prediction (cached) --------------------------------
    @st.cache_resource(show_spinner=False)
    def _predict(df_in: pd.DataFrame) -> pd.Series:
        if ml_mode:
            feats = results[model_name]["features"]
            X = pd.DataFrame(0, index=df_in.index, columns=feats)
            X["CF"]                 = df_in["CF"]
            X["CAPEX_$/kW"]         = cap_dynamic
            X["Efficiency_kWh_kg"]  = eff_dynamic
            X["Electricity_$/kWh"]  = df_in["Electricity_$/kWh"]
            X["Water_$kg"]          = df_in["Water_$kg"]
            X["CO2_$kg"]            = df_in["CO2_$kg"]
            X["Transport_$kg"]      = df_in["Transport_$kg"]
            X["Storage_$kg"]        = df_in["Storage_$kg"]
            X["Year_val"]           = year
            ...
            if tc in feats:
                X[tc] = 1
            X = sanitize(X, feats)
            preds = results[model_name]["model"].predict(X)
        else:
            crf = 0.08*(1+.08)**20 / ((1+.08)**20-1)
            kgH2= df_in["CF"]*8760/eff_dynamic
            preds = (cap_dynamic*crf/kgH2
                     + cap_dynamic*0.05*(cap_dynamic/cap_ref)/kgH2
                     + df_in["Electricity_$/kWh"]*eff_dynamic
                     + df_in["Water_$kg"]+df_in["CO2_$kg"]
                     + df_in["Transport_$kg"]+df_in["Storage_$kg"])
        return pd.Series(preds, index=df_in.index, dtype="float64")

    preds = _predict(df_sub)

    # --- helper: build single-page PDF ----------------------------------
    def build_one(state_name: str) -> tuple[str, bytes]:
        row  = df_sub.loc[state_name]
        pred = float(preds[state_name])

        # patch global header var
        old_state = globals().get("state")
        globals()["state"] = state_name

        raw = generate_pdf(row, pred, None, full=False) \
                .output(dest="S").encode("latin1")

        globals()["state"] = old_state if old_state is not None else None

        rdr = PdfReader(io.BytesIO(raw))
        if len(rdr.pages) != 1:                 # hard guard: must be 1 page
            raise ValueError(f"{state_name} PDF has {len(rdr.pages)} pages!")
        return state_name, raw

    # --- parallel build (8 threads, ~1 s) -------------------------------
    with st.spinner("Rendering 48 one-page PDFs…"):
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as exe:
            results_f = list(exe.map(build_one, lower48))
    pdf_bytes_by_state = dict(results_f)

    # --- duplicate-hash guard ------------------------------------------
    hashes = [hashlib.md5(b).hexdigest() for b in pdf_bytes_by_state.values()]
    if len(set(hashes)) != 48:
        st.error("Duplicate PDF pages detected – export aborted.")
        st.stop()

    # --- UI: choose export type ----------------------------------------
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    choice = st.radio(
        "Choose export type",
        ("Scenario PDF", "48-state ZIP", "48-state single PDF"),
        horizontal=True
    )

    if choice == "Scenario PDF":
        scenario_pdf = generate_pdf(row, pred, None, full=False) \
                          .output(dest="S").encode("latin1")
        st.download_button("Download Scenario PDF", scenario_pdf,
                           f"Scenario_{tech}_{state}_{ts}.pdf", "application/pdf")

    elif choice == "48-state ZIP":
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as z:
            for s in lower48:
                z.writestr(f"{s}_{tech}_{year}.pdf", pdf_bytes_by_state[s])
        buf.seek(0)
        st.download_button("Download 48-state ZIP", buf,
                           f"AllStates_{tech}_{year}_{ts}.zip", "application/zip")

    else:  # single 48-page PDF
        writer = PdfWriter()
        for s in lower48:
            reader = PdfReader(io.BytesIO(pdf_bytes_by_state[s]))
            writer.add_page(reader.pages[0])
        buf = io.BytesIO()
        writer.write(buf); buf.seek(0)
        st.download_button("Download 48-state PDF", buf,
                           f"AllStates_{tech}_{year}_{ts}.pdf", "application/pdf")
