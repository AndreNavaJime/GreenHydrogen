...
with tabs[2]:
    ...
    # ───────────────────────────── Feature importance (native or linear) ──────
    if ml_mode:
        if "best" in globals():
    ...
    if ml_mode and "best" in globals():
        ...
        if shap_vals is not None and explainer is not None:
            import shap, matplotlib.pyplot as plt  # local import keeps app light

            # 🔧 SAFETY GUARD – unwrap list → float64 array
            if isinstance(shap_vals, list):
                shap_vals = shap_vals[0]
            shap_vals = np.asarray(shap_vals, dtype=np.float64)

            # Align lengths (paranoia check)
            ...
            shap.summary_plot(
                shap_vals_plot,
                X_train_plot,
                feature_names=results[model_name]["features"],
                show=False,
            )
            ...
            with st.expander("Explain a single training row"):
                ...
                st.components.v1.html(
                    force_html.html(), height=300, scrolling=True
                )
        else:
            st.info(
                "SHAP not available for this estimator – showing fallback importances."
            )
