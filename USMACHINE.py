...
with tabs[3]:
    ...
    if calculation_mode == "ML Model":
        # Build feature row and predict only once
        subset = df.query("State == @state and Year == @year and Tech == @tech")
        if subset.empty:
            st.warning("No data for this scenario.")
        else:
            ...
            for feat in results[model_name]["features"]:
                if feat.startswith("Year_"):
                    row_feat[feat] = 1 if feat == f"Year_{year}" else 0

            df_feat = pd.DataFrame([row_feat]).reindex(columns=results[model_name]["features"], fill_value=0)
            raw_pred = results[model_name]["model"].predict(df_feat)[0]
            lcoh = round(max(raw_pred, 0.0), 2)

            # Prepare map DataFrame
            map_df = gdf.copy()
            map_df["LCOH ($/kg)"] = np.nan
            map_df.loc[map_df.State == state, "LCOH ($/kg)"] = lcoh
            ...
            fig_map = px.choropleth(
                ...
                hover_name="State",
                color_continuous_scale="Viridis",
                range_color=(map_df["LCOH ($/kg)"].min(), map_df["LCOH ($/kg)"].max()),
                template="plotly_dark",
                title=f"Predicted LCOH by State — Highlight: {state}",
            )
            ...
            fig_map.update_geos(scope="usa", visible=False)
        ...
        fname, col = tech_file_map[tech]
        try:
            d = pd.read_csv(fname)
            d_year = d[d.Year == year].rename(columns={col: "LCOH ($/kg)"})
            d_year["LCOH ($/kg)"] = d_year["LCOH ($/kg)"].round(2)
            m = gdf.merge(d_year[["State","LCOH ($/kg)"]], on="State", how="left")
            fig_map = px.choropleth(
                m,
                geojson=m.geometry,
                locations=m.index,
            ...
            st.error(f"Missing data file: {fname}")

    else:
        st.info("🌐 Map available only in ML Model or True Equation mode.")
