...
# --- Get scenario values from CSV
row_df = df.query("State==@state & Year==@year & Tech==@tech")
if row_df.empty:
...
cap_default = float(cap_doe) if auto_mode else float((cap_min + cap_max) / 2)
eff_default = float(eff_doe) if auto_mode else float((eff_min + eff_max) / 2)
elec_default = float(row_base["Electricity_$/kWh"])

# --- Lock sliders in TRUE MODE
if calculation_mode == "True Equation":
    cap_display = row_base["CAPEX_$/kW"]
    eff_display = row_base["Efficiency_kWh_kg"]
    elec_display = row_base["Electricity_$/kWh"]
    slider_disabled = True
else:
    cap_display = cap_default
    eff_display = eff_default
    elec_display = elec_default
    slider_disabled = auto_mode

cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max), int(cap_display), step=1, key="cap_val", disabled=slider_disabled)
eff_input = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max), float(eff_display), step=0.1, key="eff_val", disabled=slider_disabled)
elec = st.sidebar.slider("Electricity price ($/kWh)", 0.01, 0.30, float(elec_display), step=0.01, key="elec_val", disabled=slider_disabled)
...
if calculation_mode == "ML Model":
    ...
    row_input = pd.DataFrame([row])
    for col in results[model_name]["features"]:
        if col not in row_input.columns:
            row_input[col] = 0
    row_input = row_input[results[model_name]["features"]]
    pred = best.predict(row_input)[0]
else:
    def true_lcoh(cap, eff, cf, elec_price, wc, cs, tc, sc, cap_ref=500, disc=0.08):
