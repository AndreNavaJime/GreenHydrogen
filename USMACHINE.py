...
with tabs[3]:
    ...
    # local PNG-export helper (white BG, no title) – uses Kaleido
    import copy, plotly.io as pio
    def to_png(fig, scale=2):
        f = copy.deepcopy(fig)
        f.update_layout(template="plotly_white",
                        paper_bgcolor="white",
                        plot_bgcolor="white",
    ...
    if calculation_mode == "ML Model":
        ...
        # download button for the highlight map
        st.download_button(
        ...
        if show_full:
            ...
            # download button for the full choropleth
            st.download_button(
                "Download map PNG (white, no title)",
                to_png(fig_ml),
    ...
    # ───────────────────────── BASELINE-TEA VIEW ─────────────────────────
        ...
        try:
            ...
            if df_year.empty:
                ...
                # download button for Baseline map
                st.download_button(
                    "Download map PNG (white, no title)",
                    to_png(fig_te),
