...
# ──────────────────────────────────────────────────────────────────────────
# TAB 6 – Download Center (thread-safe, duplicate-free, summary page first)
# ──────────────────────────────────────────────────────────────────────────
with tabs[6]:
    ...
    # ── 1) Build df_sub: exactly one row per state ────────────────────────
    rows = []
    ...
    df_sub = pd.DataFrame(rows).set_index("State")      # 48 rows

    # ── 2) Vector prediction (cached) ─────────────────────────────────────
    @st.cache_resource(show_spinner=False)
    def _predict48(df_v: pd.DataFrame) -> pd.Series:
        if ml_mode:
    ...
    def make_state_page(st_name: str, pred_val: float) -> bytes:
        pdf = FPDF(); pdf.add_page()
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, f"{st_name} - Predicted LCOH", ln=True)
        pdf.set_font("Arial", "", 12)
        pdf.cell(0, 10, f"LCOH: ${pred_val:,.2f}/kg", ln=True)
        return pdf.output(dest="S").encode("latin1")

    # ── 4) Render the 48 state pages in parallel (no globals) ─────────────
    def _build_one(st_name: str) -> tuple[str, bytes]:
    ...
    # Sanity check: 48 distinct pages
    ...
    export_type = st.radio(
        "Choose export type",
        ("Scenario PDF", "ZIP (48 PDFs)", "Single PDF (Summary + 48 pages)"),
        horizontal=True
    )

    if export_type == "Scenario PDF":
        single_pdf = make_state_page(state, float(preds48[state]))
        st.download_button("Download Scenario PDF", single_pdf,
