...
def train_models(
    ...
    *,
    ...
    random_state: int = 42,
    show_title: bool = False,   # ← no UI title in app
    verbose_debug: bool = False,
):
    """
    TRAIN MODELS  (code anchor only; not shown in UI)

    - Drops any potential leakage columns that contain "LCOH" except 'LCOH_$kg'.
    - Fits models with a non-negative target transform (y_log = log1p(y); inverse = expm1).
    - Reports OOF metrics on the original $/kg scale; optional 80/20 hold-out.
    - Uses GroupKFold if `groups_col` exists; else KFold(shuffle=True).
    - Returns: (results_dict, X_float64). Each results[name] has keys:
        {'model','inference_model','model_prototype','features', metrics..., 'explainer','shap_values'}
    """
    ...
    # 0) leakage guard: drop any LCOH-like columns except the true label
    target_col = "LCOH_$kg"
    bad_like = [c for c in df_p.columns if ("LCOH" in c.upper()) and (c != target_col)]
    if bad_like:
        df_p = df_p.drop(columns=bad_like, errors="ignore")
        st.sidebar.warning(
            "Dropped potential leakage columns: " + ", ".join(bad_like)
        )

    # 1) engineered time feature
    if "Year" in df_p.columns:
        df_p["Year_val"] = df_p["Year"]

    # 2) one-hot Tech, drop non-predictors
    drop_cols = [c for c in ["State", target_col, "Year"] if c in df_p.columns]
    X = pd.get_dummies(
    ...
    if target_col not in df_p.columns:
        st.sidebar.error(f"Missing target column '{target_col}'.")
        return {}, None
    y = pd.to_numeric(df_p[target_col], errors="coerce")
    ...
    # ---- warn once if core drivers are missing (predictors your UI depends on)
    CORE_FEATS = {"CAPEX_$/kW", "Efficiency_kWh_kg", "Electricity_$/kWh", "CF"}
    missing_core = [f for f in CORE_FEATS if f not in X.columns]
    if missing_core:
        st.sidebar.warning(
            "Training data missing core drivers: "
            + ", ".join(missing_core)
            + ". ML scenarios may not respond to sliders."
        )

    # ---- model zoo with non-negative target via TTR ----------------------
    def _ttr(reg):
        # log1p/expm1 ensures y_hat >= 0 after inverse
    ...
    if (groups is not None) and (len(set(groups)) > 1):
        ...
        cv_note = f"{len(splits)}-fold GroupKFold"
    else:
        splitter = KFold(n_splits=max(2, oof_folds), shuffle=True, random_state=random_state)
        splits = list(splitter.split(X, y))
        cv_note = f"{len(splits)}-fold KFold (shuffled)"
    if verbose_debug:
        st.sidebar.info(f"CV: {cv_note}")
    ...
    for name, proto in models.items():
        ...
        for tr_idx, va_idx in splits:
            ...
            y_pred_oof[va_idx] = np.clip(y_hat, 0.0, None)  # non-negative
        ...
        # fully-fitted model for inference
        ...
        try:
            ...
            # unwrap to the inner regressor (inside TTR in a Pipeline)
            last_step = inference_model[-1] if hasattr(inference_model, "__getitem__") else inference_model
            ttr_obj   = getattr(last_step, "named_steps", {}).get("ttr", last_step)
        ...
        results[name] = {
            ...
            **holdout,
            ...
            "core_feats_present": [f for f in CORE_FEATS if f in X.columns],
        }

    # gentle flag if any model looks "too perfect"
    for k, v in results.items():
        if v.get("oof_r2", 0) > 0.999:
            st.sidebar.warning(f"{k}: OOF R²≈1.0 — check for leakage or duplicate rows.")
