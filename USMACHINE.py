...
with tabs[6]:
    ...
    # ───────────────── Residual plot (ML mode) ───────────────────────────
    fig_path = None
    if ml_mode:
        fig, ax = plt.subplots(figsize=(6, 3))
        ax.plot(results[model_name]["y_true"], label="True", linewidth=2)
        ax.plot(results[model_name]["preds"], label="Pred", linestyle="--")
    ...
    # Raw subset (duplicates allowed) – keep for safe row selection later
    df_sub = (
        df.query("Tech == @tech and Year == @year")
          .sort_values("State")            # deterministic tie-break
          .reset_index(drop=True)
    )

    # --- DEBUG block (comment out once verified) -------------------------
    uniq_states = df_sub["State"].unique().tolist()
    st.write("▶️ Unique states in df_sub:", len(uniq_states))
    st.write("▶️ States present:", uniq_states)
    st.write("🚩 Missing states:", sorted(set(lower48) - set(uniq_states)))
    # ---------------------------------------------------------------------

    # ---------- core LCOH calculators ------------------------------------
    def deterministic_lcoh(row_fix):
        crf  = 0.08 * (1 + 0.08)**20 / ((1 + 0.08)**20 - 1)
        kgH2 = row_fix["CF"] * 8760 / eff_dynamic
        return round(
            cap_dynamic * crf / kgH2
            + cap_dynamic * 0.05 * (cap_dynamic / cap_ref) / kgH2
            + row_fix["Electricity_$/kWh"] * eff_dynamic
            + row_fix["Water_$kg"]
            + row_fix["CO2_$kg"]
            + row_fix["Transport_$kg"]
            + row_fix["Storage_$kg"],
            4
        )

    def ml_lcoh(row_fix):
        X_row = {f: 0 for f in results[model_name]["features"]}
        X_row.update({
            "CF": row_fix["CF"],
            "CAPEX_$/kW": cap_dynamic,
            "Efficiency_kWh_kg": eff_dynamic,
            "Electricity_$/kWh": row_fix["Electricity_$/kWh"],
            "Water_$kg": row_fix["Water_$kg"],
            "CO2_$kg": row_fix["CO2_$kg"],
            "Transport_$kg": row_fix["Transport_$kg"],
            "Storage_$kg": row_fix["Storage_$kg"],
            "Year_val": year,
            "Year_val_squared": year**2,
        })
        for f in X_row:
            if f.startswith("Tech_"):
                X_row[f] = 1 if f == f"Tech_{tech}" else 0
        X_df = sanitize(pd.DataFrame([X_row]), results[model_name]["features"])
        return float(results[model_name]["model"].predict(X_df)[0])

    # ---------- PDF helpers ---------------------------------------------
    def one_page_pdf(state_name, row_fix):
        """Return one-page PDF bytes for the given state."""
        global state
        prev_state, state = state, state_name
        assert state_name == row_fix["State"], \
            f"Header mismatch: {state_name} vs {row_fix['State']}"
        pred_val = ml_lcoh(row_fix) if ml_mode else deterministic_lcoh(row_fix)
        pdf_obj  = generate_pdf(row_fix, pred_val, fig_path, full=False)
        state = prev_state
        return pdf_obj.output(dest="S").encode("latin1")

    def first_row_or_none(df_in: pd.DataFrame, state_name: str):
        """Return first matching row or None."""
        rows = df_in.query("State == @state_name")
        if rows.empty:
            return None
        if len(rows) > 1:
            st.info(f"{state_name}: {len(rows)} rows, using first.")
        return rows.iloc[0]
    ...
    if download_mode == "Scenario Report Only":
        ...
        st.download_button(
            "Download Scenario PDF", pdf_bytes,
            file_name=f"Scenario_Report_{timestamp}.pdf",
            mime="application/pdf"
        )

    # ---------- 2) Full ZIP for current state ----------------------------
        ...
        with zipfile.ZipFile(zip_buf, "w") as z:
            ...
            if isinstance(globals().get("df_out"), pd.DataFrame):
                z.writestr("predictions.csv",
                           globals()["df_out"].to_csv(index=False))
            if isinstance(globals().get("summary_stats"), pd.DataFrame):
                z.writestr("state_summary.csv",
                           globals()["summary_stats"].to_csv())
            if isinstance(globals().get("national_trend"), pd.DataFrame):
                z.writestr("national_trend.csv",
                           globals()["national_trend"].to_csv(index=False))
        zip_buf.seek(0)
        st.download_button(
            "Download Full ZIP", zip_buf,
            file_name=f"LCOH_Report_{timestamp}.zip",
            mime="application/zip"
        )

    # ---------- 3) 48 one-page PDFs → ZIP --------------------------------
    elif download_mode == "All 48 States (ZIP)":
        st.info("Building 48 one-page PDFs …")
        progress = st.progress(0)
        zip_buf  = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w") as z:
            for i, st_name in enumerate(lower48, start=1):
                progress.progress(i / 48)
                row_fix = first_row_or_none(df_sub, st_name)
                if row_fix is None:
                    st.error(f"❌ No data for {st_name}; aborting export.")
                    st.stop()
                z.writestr(f"{st_name}_{tech}_{year}.pdf",
                           one_page_pdf(st_name, row_fix.copy()))
        zip_buf.seek(0)
        st.download_button(
            "Download 48-state ZIP", zip_buf,
            file_name=f"AllStates_{tech}_{year}_{timestamp}.zip",
            mime="application/zip"
        )

    # ---------- 4) 48-page single PDF ------------------------------------
    else:  # "All 48 States (Single PDF)"
        try:
            from PyPDF2 import PdfReader, PdfWriter
        except ImportError:
            st.error("PyPDF2 missing – add `PyPDF2>=3.0` to requirements.txt.")
            st.stop()

        st.info("Compiling single 48-page PDF …")
        progress = st.progress(0)
        writer   = PdfWriter()
        for i, st_name in enumerate(lower48, start=1):
            progress.progress(i / 48)
            ...
            if row_fix is None:
                st.error(f"❌ No data for {st_name}; aborting export.")
                st.stop()
            page_pdf = one_page_pdf(st_name, row_fix.copy())
