...
with tabs[3]:
    if calculation_mode == "ML Model":
        ...
        map_df = gdf.copy()
        map_df["ColorGroup"] = map_df["State"].apply(
            lambda s: "Selected" if s == state else "Other"
        )

        fig_map = px.choropleth(
            map_df,
            geojson=map_df.geometry,
            locations="State",                       # ✅ Use State column (not index)
            color="ColorGroup",                      # ✅ Use readable label, not raw bool
            color_discrete_map={"Selected": "yellow", "Other": "#cccccc"},
            hover_name="State",                      # ✅ Show only state name
            hover_data=[],                           # ✅ Remove extra hover info
            template="plotly_dark"
        )
        ...
        # (your True Equation map remains unchanged)
        ...
            ...
            if calculation_mode == "ML Model":
                ...
                for col in features:
                    if col not in df_row:
                        df_row[col] = 0
                df_row = df_row[features]  # Reorder for sklearn

                # Predict and store result
                row["State"] = st_name
                row["LCOH ($/kg)"] = float(best.predict(df_row)[0])
