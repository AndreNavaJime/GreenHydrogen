...
with tabs[1]:
    ...
    if ml_mode and isinstance(results, dict) and len(results) > 0:
        # Build tidy metrics table from results dict (agnostic to which models are present)
        rows = []
        ...
        # Model picker — default to the first row in the sorted table
        model_options = list(stats_df.index)
        default_model = model_options[0] if len(model_options) else None
        # If external model_name exists and is valid, use it; otherwise fallback
        if ('model_name' in globals()) and (model_name in model_options):
            default_model = model_name
        model_name = st.selectbox("Select model", model_options, index=model_options.index(default_model) if default_model in model_options else 0)
        ...
        # Highlight the selected model’s metrics (robust to NaNs)
        sel = stats_df.loc[model_name]
        def _fmt(x): 
            try:
                return f"{x:.3f}" if np.isfinite(float(x)) else "n/a"
            except Exception:
                return "n/a"

        oof_str = f"R²={_fmt(sel.get('R² (OOF)', np.nan))}, RMSE={_fmt(sel.get('RMSE (OOF)', np.nan))}, MAE={_fmt(sel.get('MAE (OOF)', np.nan))}"
        holdout_avail = np.isfinite(sel.get("R² (Hold-out)", np.nan)) or np.isfinite(sel.get("RMSE (Hold-out)", np.nan)) or np.isfinite(sel.get("MAE (Hold-out)", np.nan))
        if holdout_avail:
            holdout_str = f" | Hold-out: R²={_fmt(sel.get('R² (Hold-out)', np.nan))}, RMSE={_fmt(sel.get('RMSE (Hold-out)', np.nan))}, MAE={_fmt(sel.get('MAE (Hold-out)', np.nan))}"
        else:
