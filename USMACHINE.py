...
with tabs[6]:
    ...
    def _pred_dict(rows_by_state, ml_mode, model_name):
        """Return {state: (row, pred_val)} for all 48."""
        df_vec = pd.DataFrame(rows_by_state.values())
        if ml_mode:                                           # ML branch
            X_bulk = {f: 0 for f in results[model_name]["features"]}
            X_bulk = pd.DataFrame([X_bulk] * len(df_vec))
            X_bulk["CF"]                   = df_vec["CF"]
        ...
        return {s: (rows_by_state[s], float(p))          # cast to py float
                for s, p in zip(df_vec["State"], preds)}

    state_dict = _pred_dict(rows_by_state, ml_mode, model_name)

    # ---------- cached PDF bytes ----------
    @st.cache_resource(show_spinner=False)
    def _pdf_cache(state_dict):
        pdfd = {}
        for st_name, (row, pred_val) in state_dict.items():
            pdf_obj = generate_pdf(row, pred_val, fig_path, full=False)
            pdfd[st_name] = pdf_obj.output(dest="S").encode("latin1")
        return pdfd

    pdf_bytes_by_state = _pdf_cache(state_dict)

    # ---------- helper: quick write ----------
    def add_pages_to_writer(writer):
    ...
    if download_mode == "Scenario Report Only":
        ...
        st.info("Packaging 48 one-page PDFs …")
        zip_buf = io.BytesIO()
        with zipfile.ZipFile(zip_buf, "w") as z:
            for st_name in lower48:
        ...
        st.info("Merging into single PDF …")
        writer = PdfWriter()
        add_pages_to_writer(writer)
        out_buf = io.BytesIO()
