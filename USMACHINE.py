...
with tabs[6]:
    ...
    # ---------- 1) One-page PDF for current scenario ---------------------
    ...
    # ---------- 3) One-page PDF for EVERY state → ZIP --------------------
    elif download_mode == "All 48 States (ZIP)":
        st.info("Building one-page PDFs for each state …")
        all_zip = io.BytesIO()
        with zipfile.ZipFile(all_zip, "w") as z:
            for st_name in sorted(df.State.unique()):
                ...
                prev_state, state = state, st_name          # ← patch
                pdf_state = generate_pdf(row_fix, lcoh_det, None, full=False)
        ...
        all_zip.seek(0)
        st.download_button(
            "Download 48-state ZIP", all_zip,
            file_name=f"AllStates_{tech}_{year}_{timestamp}.zip",
            mime="application/zip", key="dl_48_zip"
        )

    # ---------- 4) Single multi-page PDF for all states ------------------
        ...
        st.info("Compiling single PDF for all states …")
        writer = PdfWriter()
        ...
        single_buf.seek(0)
        st.download_button(
            "Download 48-state PDF", single_buf,
            file_name=f"AllStates_{tech}_{year}_{timestamp}.pdf",
            mime="application/pdf", key="dl_48_pdf"
        )
