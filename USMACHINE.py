...
cap = st.sidebar.slider(
    ...
    value=int((cap_min + cap_max) / 2),
    help="Capital expenditure range depends on selected technology"
)
...
eff = st.sidebar.slider(
    ...
    value=round((eff_min + eff_max) / 2, 1),
    help="Energy used per kg of H₂ produced — varies by tech"
)
...
# Adjust CAPEX dynamically based on DOE roadmap
def adjust_capex_by_year(tech: str, year: int, original_capex: float) -> float:
    targets = {
        "ALKALINE": {2022: 500, 2026: 250, 2031: 150},
        "PEM":      {2022: 1000, 2026: 250, 2031: 150},
        "SOEC":     {2022: 2500, 2026: 500, 2031: 200}
    }
    ...
    if year <= 2022:
        ...
        return round(np.interp(year, [2022, 2026], [t[2022], t[2026]]), 2)
    else:
        return round(np.interp(year, [2026, 2031], [t[2026], t[2031]]), 2)
