    ...
    # ---------- 3) 48 one-page PDFs → ZIP --------------------------------
        ...
        with zipfile.ZipFile(zip_buf, "w") as z:
            for i, st_name in enumerate(lower48, start=1):
                ...
                rows = df_sub.query("State == @st_name")
                if rows.empty:
                    st.warning(f"No data for {st_name}; skipped.")
                    continue
                if len(rows) > 1:
                    st.info(f"{st_name}: {len(rows)} rows, using first.")
                row_fix = rows.iloc[0]
                z.writestr(f"{st_name}_{tech}_{year}.pdf",
                           one_page_pdf(st_name, row_fix))
        zip_buf.seek(0)
        st.download_button(
    ...
    # ---------- 4) 48-page single PDF ------------------------------------
        ...
        for i, st_name in enumerate(lower48, start=1):
            ...
            if len(rows) > 1:
                st.info(f"{st_name}: {len(rows)} rows, using first.")
            row_fix  = rows.iloc[0]
            page_pdf = one_page_pdf(st_name, row_fix)
