...
cap_default  = cap_doe if auto_mode else st.session_state.get("cap_val", default_cap)
eff_default  = eff_doe if auto_mode else st.session_state.get("eff_val", default_eff)
elec_default = st.session_state.get("elec_val", default_elec)

# ✅ Clamp to valid ranges to avoid Streamlit slider crashes
cap_default = max(min(cap_default, cap_max), cap_min)
eff_default = max(min(eff_default, eff_max), eff_min)

# --- Sliders ---
cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max),  # 👈 ensure integers
                        value=cap_default, key="cap_val", disabled=auto_mode)
