...
if calculation_mode == "ML Model":
    ...
    cap_display = max(min(cap_default, cap_max), cap_min)
    eff_display = max(min(eff_default, eff_max), eff_min)
    elec_display = elec_default

    cap = st.sidebar.slider("CAPEX ($/kW)", int(cap_min), int(cap_max), int(cap_display), step=1, key="cap_val", disabled=auto_mode)
    eff_input = st.sidebar.slider("Efficiency (kWh/kg)", float(eff_min), float(eff_max), float(eff_display), step=0.1, key="eff_val", disabled=auto_mode)
    elec = st.sidebar.slider("Electricity price ($/kWh)", 0.01, 0.30, float(elec_display), step=0.01, key="elec_val", disabled=auto_mode)

    cap_dynamic = cap_doe if auto_mode else cap
    eff_dynamic = eff_doe if auto_mode else eff_input

else:
    st.sidebar.markdown("### 📈 Using values from data")
    st.sidebar.metric("CAPEX ($/kW)", f"{row_base['CAPEX_$/kW']:.2f}")
    st.sidebar.metric("Efficiency (kWh/kg)", f"{row_base['Efficiency_kWh_kg']:.2f}")
    st.sidebar.metric("Electricity Price ($/kWh)", f"{row_base['Electricity_$/kWh']:.4f}")
