...
# Tab 3: Predicted LCOH by State
with tabs[3]:
    st.subheader("Predicted LCOH by State")

    if calculation_mode == "ML Model":
        # Build features for selected state only
        ...
        if subset.empty:
            ...
            df_feat = pd.DataFrame([row_feat])
            df_feat = df_feat.reindex(columns=results[model_name]["features"], fill_value=0)
            raw_pred = results[model_name]["model"].predict(df_feat)[0]
            pred_val = float(max(raw_pred, 0.0))
            lcoh = round(pred_val, 2)

            # Build map data: only current state
            map_df = gdf.copy()
            ...
            fig_map = px.choropleth(
                ...
                color_continuous_scale="Viridis",
                template="plotly_dark",
                title=f"Predicted LCOH by State – Highlighting {state}"
            )
            fig_map.update_geos(scope="usa", visible=False)
            fig_map.update_coloraxes(colorbar_tickformat=".2f", showscale=True)
        ...
        try:
            ...
            if df_year.empty:
                ...
                map_df = gdf.merge(df_year[["State", "LCOH ($/kg)"]], on="State", how="left")
                fig_map = px.choropleth(
                    map_df,
                    geojson=map_df.geometry,
                    locations=map_df.index,
