...
def train_models(df: pd.DataFrame):
    ...
    def _clean_numeric_df(Xin: pd.DataFrame) -> pd.DataFrame:
        """Strip bracketed numbers like '[4.144E0]', commas, and spaces; return float64."""
        Xout = Xin.copy()

        # apply only to object-like columns
        obj_cols = [c for c in Xout.columns if Xout[c].dtype == "object"]
        if obj_cols:
            # vectorized regex cleanup
            pat = re.compile(r'^\s*\[?\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\s*\]?\s*$')
            for c in obj_cols:
                s = Xout[c].astype(str).str.replace(",", "", regex=False).str.strip()
                # keep values that match a number (with optional brackets); else NaN
                s = s.apply(lambda v: pat.sub(r"\1", v) if pat.match(v) else np.nan)
                Xout[c] = pd.to_numeric(s, errors="coerce")

        # coerce everything numeric, kill infs, fill with medians → 0, cast float64
    ...
    for name, model in models.items():
        ...
        # --- SHAP ------------------------------------------------------------
        try:
            # Always re-sanitize the background slice
            bg_pd = _clean_numeric_df(X_tr.sample(min(200, len(X_tr)), random_state=42))
            bg_np = bg_pd.to_numpy(dtype=np.float64, copy=False)
            ...
            if lname in {"linear", "ridge"}:
                ...
                explainer = shap.TreeExplainer(model, data=bg_np, feature_perturbation="interventional")
                shap_raw = explainer.shap_values(bg_np[:500])
            else:
            ...
            shap_vals = (np.asarray(shap_raw[0], dtype=np.float64)
                         ...
                         else np.asarray(shap_raw, dtype=np.float64))
