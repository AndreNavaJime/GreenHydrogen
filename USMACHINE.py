    ...
    for st_name in sorted(df.State.dropna().unique()):
        ...
        for c, e in product(cap_values, eff_values):
            ...
            if ml_mode and _best is not None and features:
                ...
                # Deterministic TEA fallback (kept simple and stable)
                ...
                # Levelized terms per kg
                capex_term = (c * crf) / kgH2_per_kWyr
                opex_term  = (0.05 * c) / kgH2_per_kWyr   # 5% of CAPEX per year
    ...
    if national_df.empty:
        ...
        if calculation_mode == "ML Model":
            ...
            for y in years:
                ...
                # Safe column access with fallback to 0.0
                wc = float(subset["Water_$kg"].mean()) if "Water_$kg" in subset.columns else 0.0
                cs = float(subset["CO2_$kg"].mean()) if "CO2_$kg" in subset.columns else 0.0
                tc = float(subset["Transport_$kg"].mean()) if "Transport_$kg" in subset.columns else 0.0
                sc = float(subset["Storage_$kg"].mean()) if "Storage_$kg" in subset.columns else 0.0
