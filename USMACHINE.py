...
def train_models(df):
    ...
    if os.path.exists("results.pkl"):
        ...
        models = {
            ...
            "XGBoost": XGBRegressor(n_estimators=150, verbosity=0),
    ...
    return results, X_train, y_test

results, X_train, y_test = train_models(df)

# --- Sidebar Inputs ---
st.sidebar.header("🔧 Scenario Simulator")
model_names = list(results.keys())
best_model_name = st.sidebar.selectbox("Choose Model", model_names, index=model_names.index("XGBoost"))
...
with tabs[0]:
    st.markdown(f"### 🧪 Predicted LCOH → **${pred:.2f}/kg** using **{best_model_name}**")
...
with tabs[2]:
    fi = pd.Series(getattr(best, "feature_importances_", None), index=X_train.columns)
    if fi.isnull().all(): fi = None
    if fi is not None:
        fig_bar = px.bar(x=fi.sort_values(), y=fi.sort_values().index, orientation="h",
                         color=fi.sort_values().values, color_continuous_scale="Viridis",
                         title="🔍 Feature Importance", template="plotly_dark")
        fig_bar.update_layout(height=450)
        st.plotly_chart(fig_bar, use_container_width=True)

    pivot = df.groupby(["Tech", "Year"])["LCOH_$kg"].mean().reset_index().pivot(
        index="Tech", columns="Year", values="LCOH_$kg"
    )
    pivot = pivot.sort_index(axis=1)
    fig_heat = px.imshow(pivot, color_continuous_scale="Viridis",
                         labels=dict(y="Tech", x="Year", color="LCOH ($/kg)"),
...
with tabs[3]:
    ...
    fig_map = px.choropleth(merged, geojson=merged.geometry, locations=merged.index, color="LCOH_$kg",
                            hover_name="State", color_continuous_scale="Viridis",
                            title=f"LCOH by State ({year})", template="plotly_dark")
    fig_map.update_geos(scope="usa", showcoastlines=True, coastlinecolor="white",
                        showland=True, landcolor="#1e1e1e", showlakes=True, lakecolor="#1e1e1e")
    st.plotly_chart(fig_map, use_container_width=True)

with tabs[4]:
    st.markdown("📥 Download predictions & stats")
    df_out = pd.DataFrame({"True":y_test.values, "Predicted":results[best_model_name]["preds"], 
                            "Error":y_test.values - results[best_model_name]["preds"]})
